...
void RLL::handleDataMessage(cPacket *msg) {
    ...
    RLLClusterTableEntry *tempEntry = clusterTable->getEntryByShrtAddr(
            ctrl->getSrcAddr());
    if ((temp->getBurstId() > nLastBurstId
            || temp->getMessageId() > nLastMessageId) && tempEntry != NULL) { //First send it to application layer
...
void RLL::handle_MLME_BEACON_confirm(cMessage *msg) {
    Ieee802154eNetworkCtrlInfo *beaconCon = check_and_cast<
            Ieee802154eNetworkCtrlInfo *>(msg);
    double time = uniform(
            pow(2, beaconCon->getBeaconOrder())
...
void RLL::SCHEDULE_response(cMessage *msg) {
    ...
    scheduleFrame->setMyChannelList(
            clusterTable->generateChannelList(tempFrame->getSrcAddr().getInt(),
                    myChannel));
    scheduleFrame->setSrcName(getParentModule()->getName());
    scheduleFrame->setSrcIndex(getParentModule()->getIndex());
...
void RLL::handle_BEACON_WAIT_timer(cMessage *msg) {
    ...
    if (nScanCounter < (numChannel + 1)) {
        ...
        nScanCounter++;
        if (!BeaconScanTimer->isScheduled())
            scheduleAt(simTime() + 1.5, BeaconScanTimer);
