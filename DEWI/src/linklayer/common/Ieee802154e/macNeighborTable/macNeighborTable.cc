...
void macNeighborTable::editNeighbor(macNeighborTableEntry *entry)
{
    ...
    {
	if(ev.isGUI())
	    EV << nodename << "not possible to edit Neighbor:" << entry->getNeighborId() << ". Neighbor doesn't exist!!!" << "\n";
    ...
    if(entry->getNeighborSize() != getNeighborById(entry->getNeighborId())->getNeighborSize() && entry->getNeighborSize() != -1)
    {
	if(ev.isGUI())
