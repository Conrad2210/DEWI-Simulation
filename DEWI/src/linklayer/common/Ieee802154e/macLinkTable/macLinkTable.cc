...
bool macLinkTable::existLink(UINT_16 address)
{
    ...
    for (int i = 0; i < (int)idToLink.size(); i++)
    {
        if (idToLink[i]->getNodeAddress() == address
                && (idToLink[i]->getLinkOption() == LNK_OP_TRANSMIT
                        || idToLink[i]->getLinkOption() == LNK_OP_SHARED
                        || idToLink[i]->getLinkOption() == LNK_OP_SHARED_RECEIVE))
            return true;
    }
    return false;
}
...
bool macLinkTable::isTimescource(UINT_16 address)
{
    ...
    {
        if (idToLink[i]->getNodeAddress() == address
                && idToLink[i]->getLinkOption() == LNK_OP_RECEIVE_TIMEKEEPING)
            return true;
    }
    return false;
}
...
bool macLinkTable::addLink(macLinkTableEntry *entry)
{
    ...
    if (slotframeTable->getSlotframeById(entry->getSlotframeId()) == NULL)
    {
        if (ev.isGUI())
        {
            EV << getParentModule()->getParentModule()->getFullName() << "not possible to add link,  slotframe:"
                    << entry->getSlotframeId() << "\n";
        }
        return false;
    }
    else
    {
        ...
        if (entry->getLinkId() == -1)
        {
            if (ev.isGUI())
            {
                EV << getParentModule()->getParentModule()->getFullName()
                        << " addLink(): no LinkId set, add Link is not possible" << "\n";
            }
            return false;
        }
        ...
        if (getLinkByIdAndSlotframe(entry->getLinkId(), entry->getSlotframeId()) != NULL)
        {
            ...
            {
                EV << getParentModule()->getParentModule()->getFullName()
                        << "not possible to add link, link already registered for this slotframe" << "\n";
            }
            return false;
        }
...
//        }
        ...
        if (entry->getTimeslot() > slotframeTable->getSlotframeById(entry->getSlotframeId())->getSlotframeSize())
        {
            ...
            {
                EV << getParentModule()->getParentModule()->getFullName()
                        << "not possible to add link, is not available for this slotframeId" << "\n";
            }
            return false;
        }
...
void macLinkTable::editLink(macLinkTableEntry *entry)
{
    ...
    if (getLinkById(entry->getLinkId()) == NULL)
    {
        if (ev.isGUI())
            EV << nodename << "not possible to edit link:"
                    << entry->getLinkId() << ", because Link doesn't exist" << "\n";
    ...
    if (entry->getChannelOffset() != getLinkById(entry->getLinkId())->getChannelOffset()
            && entry->getChannelOffset() != -1)
    {
        if(ev.isGUI())
            EV << nodename << ": Edited link: " << entry->getLinkId()
                << "; Changed channel offset" << endl;
    ...
    if (entry->getLinkOption() != getLinkById(entry->getLinkId())->getLinkOption()
            && entry->getLinkOption() != LNK_OP_undefinedOption)
    {
        EV << nodename << ": Edited link: " << entry->getLinkId()
                << "; Changed Link Option" << endl;
