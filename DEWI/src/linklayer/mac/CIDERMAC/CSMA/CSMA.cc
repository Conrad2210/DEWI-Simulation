...
void CSMA::initialize(int stage) {
    if (stage == 0) {
        ...
        EV << "queueLength = " << queueLength << " bitrate = " << bitrate
    ...
    if (macPkt->getDstAddr() == getMacAddr()) {
        if (!useMACAcks) {
            ...
            if (strcmp(macPkt->getName(), "CSMA-Ack") != 0) {
                ...
                Ieee802154eFrame * firstPacket =
                        static_cast<Ieee802154eFrame *>(queueModule->requestSpcPacket());
...
void CSMA::updateStatusIdle(t_mac_event event, cMessage* msg) {
    ...
    case EV_SEND_REQUEST:
        if (queueModule->getQueueSize() <= (int)queueLength) {
            ...
            EV
                      << "(1) FSM State IDLE_1, EV_SEND_REQUEST and [TxBuff avail]: startTimerBackOff -> BACKOFF."
                      << endl;
            ...
            EV
                      << "(12) FSM State IDLE_1, EV_SEND_REQUEST and [TxBuff not avail]: dropping packet -> IDLE."
                      << endl;
    ...
    case EV_DUPLICATE_RECEIVED:
        EV
                  << "(15) FSM State IDLE_1, EV_DUPLICATE_RECEIVED: setting up radio tx -> WAITSIFS."
                  << endl;
    ...
    case EV_FRAME_RECEIVED:
        EV
                  << "(15) FSM State IDLE_1, EV_FRAME_RECEIVED: setting up radio tx -> WAITSIFS."
                  << endl;
    ...
    case EV_BROADCAST_RECEIVED:
        EV << "(23) FSM State IDLE_1, EV_BROADCAST_RECEIVED: Nothing to do."
                  << endl;
        sendUp(decapsMsg(static_cast<Ieee802154eFrame *>(msg)));
        delete msg;
        break;
...
void CSMA::updateStatusCCA(t_mac_event event, cMessage* msg) {
    ...
    case EV_TIMER_CCA: {
        ...
        if (isIdle) {
            ...
            PLME_SET_TRX_STATE_request(phy_TX_ON);
            //phy->setRadioState(Radio::TX);
            Ieee802154eFrame * mac = check_and_cast<Ieee802154eFrame *>(
                    queueModule->requestSpcPacket());
            //sendDown(msg);
...
void CSMA::updateStatusNotIdle(cMessage* msg) {
    EV << "(20) FSM State NOT IDLE, EV_SEND_REQUEST. Is a TxBuffer available ?"
              << endl;


    if (queueModule->getQueueSize() <= queueLength) {
        queueModule->insertInQueue(msg);
        EV << "(21) FSM State NOT IDLE, EV_SEND_REQUEST"
                  << " and [TxBuff avail]: enqueue packet and don't move."
                  << endl;
    } else {
        // queue is full, message has to be deleted
...
void CSMA::manageMissingAck(t_mac_event event, cMessage* msg) {
    if (txAttempts < macMaxFrameRetries + 1) {
        // increment counter
        txAttempts++;
        EV << "I will retransmit this packet (I already tried " << txAttempts
                  << " times)." << endl;
    } else {
        // drop packet
        EV << "Packet was transmitted " << txAttempts
                  << " times and I never got an Ack. I drop the packet."
                  << endl;
        queueModule->deleteMsgFromQueu(queueModule->requestSpcPacket());
        txAttempts = 0;
        //mac->setName("MAC ERROR");
...
void CSMA::startTimer(t_mac_timer timer) {
    if (timer == TIMER_BACKOFF) {
        ...
        ASSERT(useMACAcks);
        simtime_t ackDuration =
                ((double) (ackLength + (def_phyHeaderLength * 8))) / bitrate;
        EV << "(startTimer) rxAckTimer value=" << macAckWaitDuration << " + "
                  << ackDuration << endl;
...
bool CSMA::toParent(Ieee802154eFrame* msg) {
    if (((msg->getFrmCtrl().dstAddrMode == defFrmCtrl_AddrMode16)
            && (msg->getDstAddr().getInt() == mpib.macCoordShortAddress))
            || ((msg->getFrmCtrl().dstAddrMode == defFrmCtrl_AddrMode64)
                    && (msg->getDstAddr() == mpib.macCoordExtendedAddress)))
        return true;
    else
        return false;
}
...
int CSMA::calFrmByteLength(Ieee802154eFrame* msg) {
    ...
    Ieee802154eFrameType frmType = Ieee802154eFrameType(
            msg->getFrmCtrl().frameType);
    int byteLength = 0; // MHR + MAC payload + MFR
    int MHRLength = calMHRByteLength(msg->getFrmCtrl().srcAddrMode,
            msg->getFrmCtrl().dstAddrMode);
    ...
    if (frmType == Ieee802154e_BEACON)  // Std 802.15.4-2006 (figure 10) page 21
        ...
        if (msg->getFrmCtrl().frameVersion == Ieee802154_compatible) {
            ...
            Ieee802154eEnhancedAckFrame* enhAckFrm = check_and_cast<
                    Ieee802154eEnhancedAckFrame *>(msg);
...
void CSMA::PLME_SET_TRX_STATE_request(PHYenum state) {
    EV << "[MAC]: sending PLME_SET_TRX_STATE_request <" << state
              << "> to PHY layer" << endl;
