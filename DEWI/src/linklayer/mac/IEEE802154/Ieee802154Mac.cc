...
def_macBeaconTxTime, 0/*def_macBSN*/, MACAddress(def_macCoordExtendedAddress), def_macCoordShortAddress,
...
{
    EV << getParentModule()->getParentModule()->getFullName() << ": initializing Ieee802154Mac, stage=" << stage
            << endl;
    ...
    if (0 == stage)
    {
        ...
        beaconForwarding = par("beaconForwarding");
        if (beaconForwarding)
        {
            int tmp = this->getParentModule()->getParentModule()->getIndex();
            srand(rand());
            if (tmp != 0)
                isCoor = rand() % 2 == 1 ? true : false;
        }
    ...
    else if (2 == stage)
    {
        ...
        if (isPANCoor)
        {
            ...
            ev << "BO = " << (int) mpib.macBeaconOrder << ", BI = "
                    << (int) aBaseSuperframeDuration * (1 << mpib.macBeaconOrder) / phy_symbolrate << " s; SO = "
                    << (int) mpib.macSuperframeOrder << ", SD = "
                    << (int) aBaseSuperframeDuration * (1 << mpib.macSuperframeOrder) / phy_symbolrate
                    << " s; duty cycle = " << pow(2.0, (mpib.macSuperframeOrder - mpib.macBeaconOrder)) * 100 << "%"
                    << endl;
            ev << "There are " << (int) aBaseSuperframeDuration * (1 << mpib.macSuperframeOrder) << " symbols ("
                    << (int) aBaseSuperframeDuration * (1 << mpib.macSuperframeOrder) / aUnitBackoffPeriod
...
void Ieee802154Mac::handleMessage(cMessage* msg)
{
    ...
    {
        if (msg->getKind() == 0)
            error("[MAC]: message '%s' with length==0 is supposed to be a primitive, but msg kind is also zero",
                    msg->getName());
...
void Ieee802154Mac::handleUpperMsg(cPacket* msg)
{
    ...
    if (taskP.taskStatus(TP_MCPS_DATA_REQUEST))
    {
        EV << "[MAC]: an " << msg->getName() << " (#" << numUpperPkt
                << ") received from the upper layer, but drop it due to busy MAC" << endl;
        delete msg;
        numUpperPktLost++;
        reqtMsgFromIFq();
        return;
...
//            destAddr = macModule->getMacAddr();
        ...
        {
            destAddr = destAddr.BROADCAST_ADDRESS;
            gtsTX = false; //send in the CAP
            indirectTX = false; //send direct
        }
        else if (destAddr.equals(mpib.macCoordExtendedAddress) || destAddr.equals((IE3ADDR) mpib.macCoordShortAddress))
        {
            if (notAssociated)
            {
                ...
                MCPS_DATA_confirm(mac_INVALID_ADDRESS);
                delete msg;
                delete cInfo;
                numUpperPktLost++;
                reqtMsgFromIFq();
            ...
            else if (dataTransMode == 3)
            {
                ...
                if (gtsStartSlot == 0 || isRecvGTS)
                {
                    EV << "[MAC]: an " << msg->getName()
                            ...
                            << endl;
                    MCPS_DATA_confirm(mac_INVALID_GTS);
                    delete msg;
                    delete cInfo;
                    numUpperPktLost++;
                    reqtMsgFromIFq();
        ...
        else // packet for device
        {
            ...
            if (mpib.macShortAddress != 0xFFFE) // device is associated to a PAN coordinator
            {
                ...
                if (mpib.macShortAddress != 0xfffe && deviceList.find(getShortAddress(destAddr)) == deviceList.end())
                {
                    EV << "[MAC]: an " << msg->getName() << " destined for the device with MAC address " << destAddr
    ...
    else
    {
        ...
        if (cInfo->getToParent())
        {
            ...
            else if (dataTransMode == 3) // GTS mode
            {
                ...
                //check if transmit GTS available
                if (gtsStartSlot == 0 || isRecvGTS)
                {
                    EV << "[MAC]: an " << msg->getName()
                            << " requesting GTS transmission destined for the PAN Coordinator received from the upper layer, but drop it due to no transmit GTS allocated by the PAN coordinator yet"
    ...
    defFrmCtrl_AddrMode16, mpib.macPANId, destAddr, msg, (Ieee802154TxOption) dataTransMode);
...
void Ieee802154Mac::handleLowerMsg(cPacket* msg)
{
    ...
    if (!frame)
    {
        EV << "[MAC]: message from physical layer (" << msg->getClassName() << ")" << msg->getName()
                << " is not a subclass of Ieee802154Frame, drop it" << endl;
        delete frame;
        return;
    }
...
// check timing for GTS (debug)
    if (frmType == Ieee802154_DATA && frame->getIsGTS())
    {
        if (isPANCoor)
        {
            ...
            if (indexCurrGts == 99 || gtsList[indexCurrGts].isRecvGTS
                    || gtsList[indexCurrGts].devShortAddr != getShortAddress(frame->getSrcAddr()))
...
// temporary solution for association process, to be modified in later version
    if (notAssociated && !isPANCoor)     // this is my first rxed beacon, associate with this one,
    {
...
// if GTS, calculate the required GTS length for transmitting pkts with constant length
        if (dataTransMode == 3)
        {
            ...
            EV << "[GTS]: gtsTransDuration = " << gtsTransDuration << " s, duration of one GTS slot = " << tmpf << " s"
                    << endl;
...
// if my GTS is not the first one in the CFP, should turn radio off at the end of CAP using finalCAPTimer
        if (gtsStartSlot != rxSfSpec.finalCap + 1)
        {
            ...
            w_time = tmpf - now;
            EV << "[GTS]: my GTS is not the first one in the CFP, schedule a timer to turn off radio at the end of CAP"
                    << endl;
...
//only handle late ack. for data packet not in GTS
        ...
        if (backoffStatus == 99)
        {
            EV << "[MAC]: this is a late ACK for " << txData->getName() << ":#" << (int) txData->getBdsn()
                    << ", stop retrying" << endl;
            backoffStatus = 0;
            csmacaCancel();
        }
...
//perform further filtering only if the PAN is currently not in promiscuous mode
    ...
    {
//check packet type
        if ((frmType != Ieee802154_BEACON) && (frmType != Ieee802154_DATA) && (frmType != Ieee802154_ACK)
...
//check source PAN ID for beacon frame
        if ((frmType == Ieee802154_BEACON) && (mpib.macPANId != 0xffff)        // associated
...
// it's called hidden, because its dstAddrMode is still set to 0
        if (frmType == Ieee802154_ACK)
        {
            if (frmCtrl.dstAddrMode == defFrmCtrl_AddrMode16
                    && frame->getDstAddr().getInt() != getShortAddress(aExtendedAddress))
...
void Ieee802154Mac::resetTRX()
{
    ...
    if ((mpib.macBeaconOrder != 15) || (rxBO != 15))  //beacon enabled
    {
        ...
        {
            EV << "[MAC]: it's now in outgoing active period (as a coordinator), should stay awake and turn on receiver"
                    << endl;
            t_state = phy_RX_ON;
        }
...
{
    EV << "[MAC]: a PLME_SET_TRX_STATE_confirm with " << status << " reveived from PHY, the requested state is "
            << trx_state_req << endl;
    simtime_t delay;
...
/***************************** <SSCS-MAC Primitive> *****************************/
//-------------------------------------------------------------------------------/
void Ieee802154Mac::MCPS_DATA_request(uint16_t SrcAddrMode, UINT_16 SrcPANId, IE3ADDR SrcAddr, uint16_t DstAddrMode,
        UINT_16 DstPANId, IE3ADDR DstAddr, cPacket* msdu, Ieee802154TxOption TxOption)
{
...
// check 3: if the entire transmission can be finished before the end of current CAP
    ...
    if (!toParent(tmpCsmaca))    // as a coordinator
    {
        if (mpib.macBeaconOrder != 15)      // beacon enabled as a coordinator
        {
            ...
            if (now >= t_fCAP)      // Check 1
            {
                ...
                EV
                        << "[CSMA-CA]: cannot porceed because it's now NOT in CAP (outgoing), resume at the beginning of next CAP"
                        << endl;
                return false;
            }
            else                    // Check 2
    ...
    else        // as a device
    {
        if (rxBO != 15)     // beacon enabled as a device
        {
            ...
            if (now >= t_fCAP)     // Check 1
            {
                ...
                bPeriodsLeft = 0;   // rechoose random backoff
                EV
                        << "[CSMA-CA]: cannot porceed because it's now NOT in CAP (incoming), resume at the beginning of next CAP"
                        << endl;
                return false;
            }
            else
    ...
    {
        EV
                << "[CSMA-CA]: cannot porceed because the chosen num of backoffs cannot finish before the end of current CAP, resume at the beginning of next CAP"
                << endl;
...
simtime_t Ieee802154Mac::getFinalCAP(char trxType)
{
    ...
    else                                                    // check rx CAP
    {
        if (rxBO != 15)                         // beacon enabled
        {
            ...
            {
                EV << "[MAC]: during receiving a beacon, now is: " << now << ", last beacon is rceived at " << bcnRxTime
                        << endl;
                while (rxCAP < now)
                    rxCAP += rxBI;
            }
...
void Ieee802154Mac::startBcnTxTimer(bool txFirstBcn, simtime_t startTime)
{
    ...
    {
        wtime = (aBaseSuperframeDuration * (1 << mpib.macBeaconOrder) - Ieee802154Phy::aTurnaroundTime)
                / phy_symbolrate;
        if (bcnTxTimer->isScheduled())
            cancelEvent(bcnTxTimer);
        scheduleAt(now + wtime, bcnTxTimer);
            ...
            case 3:
                if (pStatus == phy_SUCCESS) //ack. received
                {
                    ...
                    EV << "[GTS]: ACK for " << txGTS->getName() << ":#" << (int) txGTS->getBdsn()
                            << " received successfully, turn off radio now" << endl;
                    PLME_SET_TRX_STATE_request(phy_FORCE_TRX_OFF);
                    EV << "[GTS]: need to delay IFS before next GTS transmission can proceed" << endl;
                    taskP.taskStep(task)++;strcpy
...
bool Ieee802154Mac::toParent(Ieee802154Frame* frame)
{
    if (((frame->getFrmCtrl().dstAddrMode == defFrmCtrl_AddrMode16)
            && (frame->getDstAddr().getInt() == mpib.macCoordShortAddress))
...
// should turn on radio receiver aTurnaroundTime symbols berfore GTS starts, if it is a transmit GTS relative to device
    ...
    EV << "[GTS]: schedule for starting of GTS index: " << index_gtsTimer << ", slot:#"
            << (int) gtsList[index_gtsTimer].startSlot << " with " << w_time << " s" << endl;
    startGtsTimer(w_time);
}
...
// add new GTS descriptor
    ...
    EV << "[GTS]: add a new GTS descriptor with index = " << (int) gtsCount << " for MAC address = " << macShortAddr

    << ", startSlot = " << (int) startSlot << ", length = " << (int) length << ", isRecvGTS = " << isReceive << endl;
    gtsCount++;
