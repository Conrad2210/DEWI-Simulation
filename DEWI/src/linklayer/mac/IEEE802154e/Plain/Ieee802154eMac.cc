...
def_macShortAddress,
def_macSuperframeOrder, 0 /*def_macSyncSymbolOffset*/,
        0 /*def_macTimestampSupported*/,
        def_macTransactionPersistenceTime, 0 /*def_macTxControlActiveDuration*/,
        0 /*def_macTxControlPauseDuration*/,
        def_macTxTotalDuration,

        def_macFrameCounter,
        def_macAutoRequestSecurityLevel,
        def_macAutoRequestKeyIdMode,
        def_macAutoRequestKeyIndex,
        MACAddress::BROADCAST_ADDRESS64/*macPANCoordExtendedAddress*/,
...
string Ieee802154eMac::frameTypeTxt[] = { "unknown type", "Ieee802154e_BEACON",
        "Ieee802154e_DATA", "Ieee802154e_ACK", "Ieee802154e_CMD",
        "Ieee802154e_LLDN", "Ieee802154e_MULTI", "Ieee802154e_Asscociation" };

/**@author: 2014    Stefan Reis
 * for the debug output */
string Ieee802154eMac::stateTxt[] = { "phy_BUSY", "phy_BUSY_RX", "phy_BUSY_TX",
        "phy_FORCE_TRX_OFF", "phy_IDLE", "phy_INVALID_PARAMETER", "phy_RX_ON",
        "phy_SUCCESS", "phy_TRX_OFF", "phy_TX_ON", "phy_UNSUPPORT_ATTRIBUTE",
        "phy_READ_ONLY" };

/**@author: 2014    Stefan Reis
 * for the debug output */
string Ieee802154eMac::paraTxt[] = { "PHY_INVALID_PARAMETER",
        "PHY_CURRENT_CHANNEL", "PHY_CHANNELS_SUPPORTED", "PHY_TRANSMIT_POWER",
        "PHY_TX_POWER_TOLERANCE", "PHY_TX_POWER", "PHY_CCA_MODE",
        "PHY_CURRENT_PAGE", "PHY_MAX_FRAME_DURATION", "PHY_SHR_DURATION",
        "PHY_SYMBOLS_PER_OCTET", "PHY_PREAMBLE_SYMBOL_LENGTH",
        "PHY_UWB_DATA_RATES_SUPPORTED", "PHY_CSS_LOW_DATA_RATE_SUPPORTED",
        "PHY_UWB_COU_SUPPORTED", "PHY_UWB_CS_SUPPORTED",
        "PHY_UWB_LCP_SUPPORTED", "PHY_UWB_CURRENT_PULSE_SHAPE",
        "PHY_UWB_COU_PULSE", "PHY_UWB_CS_PULSE", "PHY_UWB_LCP_WEIGHT_1",
        "PHY_UWB_LCP_WEIGHT_2", "PHY_UWB_LCP_WEIGHT_3", "PHY_UWB_LCP_WEIGHT_4",
        "PHY_UWB_LCP_DELAY_2", "PHY_UWB_LCP_DELAY_3", "PHY_UWB_LCP_DELAY_4",
        "PHY_RANGING", "PHY_RANGING_CRYSTAL_OFFSET", "PHY_RANGING_DPS",
        "PHY_CURRENT_CODE", "PHY_NATIVE_PRF", "PHY_UWB_SCAN_BINS_PER_CHANNEL",
        "PHY_UWB_INSERTED_PREAMBLE_INTERVAL", "PHY_TX_RMARKER_OFFSET",
        "PHY_RX_RMARKER_OFFSET", "PHY_RFRAME_PROCESSING_TIME",
        "PHY_CCA_DURATION" };
...
void Ieee802154eMac::commonInitialize() {
    ...
    slotframeTable = check_and_cast<IMacSlotframeTable *>(
            getModuleByPath(par("macSlotframeTableModule")));
    // get a pointer to the LinkTable Module
    linkTable = check_and_cast<IMacLinkTable *>(
            getModuleByPath(par("macLinkTableModule")));
    // get a pointer to the TimeslotTable Module
    timeslotTable = check_and_cast<IMacTimeslotTable *>(
            getModuleByPath(par("macTimeslotTableModule")));
    ...
    neighborTable = check_and_cast<IMacNeighborTable *>(
            getModuleByPath(par("macNeighborTableModule")));
...
void Ieee802154eMac::initialize(int stage) {
    EV << getParentModule()->getParentModule()->getFullName()
              << ": initializing Ieee802154eMac, stage=" << stage << endl;
...
//        ack4Gts                 = par("ack4Gts");
        ...
        if (mpib.macBeaconOrder > 15
                || mpib.macSuperframeOrder > mpib.macBeaconOrder)
        ...
        if (dataTransMode != 1 && dataTransMode != 2 && dataTransMode != 3) // Std IEEE 802.15.4-2011 - 4.5.2 Data transfer model; page 13
            error(
                    "[MAC]: data transfer mode = %d is not supported in the model!",
                    dataTransMode);
        ...
        radioModule =
                gate("lowerLayerOut")->getNextGate()->getOwnerModule()->getId();
    } else if (1 == stage) {
        // initialize timers
        ...
        ackTimeoutTimer = new cMessage("ackTimeoutTimer",
                MAC_ACK_TIMEOUT_TIMER);
        txAckBoundTimer = new cMessage("txAckBoundTimer",
                MAC_TX_ACK_BOUND_TIMER);
        txCmdDataBoundTimer = new cMessage("txCmdDataBoundTimer",
        ...
        // TSCH transmission
        tsCCAOffsetTimer = new cMessage("tsCCAOffsetTimer",
                MAC_TS_CCA_OFFSET_TIMER);
        tsTxOffsetTimer = new cMessage("tsTxOffsetTimer",
                MAC_TS_TX_OFFSET_TIMER);
        tsRxAckDelayTimer = new cMessage("tsRxAckDelayTimer",
                MAC_TS_RX_ACK_DELAY_TIMER);
        tsAckWaitTimer = new cMessage("tsAckWaitTimer", MAC_TS_ACK_WAIT_TIMER);
        // TSCH receiving
        tsRxOffsetTimer = new cMessage("tsRxOffsetTimer",
                MAC_TS_RX_OFFSET_TIMER);
        tsRxWaitTimer = new cMessage("tsRxWaitTimer", MAC_TS_RX_WAIT_TIMER);
        tsTxAckDelayTimer = new cMessage("tsTxAckDelayTimer",
        ...
        EV << "mpib.macMaxFrameRetries initialized with: "
                  << (int) mpib.macMaxFrameRetries << endl;

        if (mpib.macTSCHenabled)
            EV << "The node is running in the IEEE Std 802.15.4e-2012 TSCH mode"
                      << endl;
        else
            EV << "The node is running in the IEEE Std 802.15.4-2006/2011 mode"
                      << endl;

        // start a PAN Coordinator or a Device
        if (isPANCoor) { // Start PAN Coordinator - Std 802.15.4-2006 (7.5.2.3.1) page 177
                         // check name
...
//		error("[MAC]: name of PAN coordinator does not match!");
            ...
            EV
                      << "************************ PAN Coordinator Parameters ************************"
                      << endl;
            EV << getParentModule()->getParentModule()->getFullName()
                      << " is the PAN coordinator!" << endl;
            EV << "Channel Number: " << ppib.phyCurrentChannel << "; bit rate: "
                      << phy_bitrate / 1000 << " kb/s; symbol rate: "
                      << phy_symbolrate / 1000 << " ksymbol/s" << endl;

            if (mpib.macTSCHenabled) {
                EV << "macMinBE = " << (int) mpib.macMinBE << ", macMaxBE "
                          << (int) mpib.macMaxBE << endl;
            } else {
                EV << "BO = " << (int) mpib.macBeaconOrder << ", BI = "
                          << (int) aBaseSuperframeDuration
                                  * (1 << mpib.macBeaconOrder) / phy_symbolrate
                          << " s; SO = " << (int) mpib.macSuperframeOrder
                          << ", SD = "
                          << (int) aBaseSuperframeDuration
                                  * (1 << mpib.macSuperframeOrder)
                                  / phy_symbolrate << " s; duty cycle = "
                          << pow(2.0,
                                  (mpib.macSuperframeOrder - mpib.macBeaconOrder))
                                  * 100 << "%" << endl;
                EV << "There are "
            ...
            if (mpib.macTSCHenabled) { // Std 802.15.4e-2012
...
//                scheduleAt(simTime()+ panStartTime, asnTimer);
            } else { // Std 802.15.4-2011
                PLME_SET_TRX_STATE_request(phy_TX_ON);

                cMessage* startPANCoorTimer = new cMessage("startPANCoorTimer",
                        START_PAN_COOR_TIMER);
...
void Ieee802154eMac::finish() {
    ...
    if (mpib.macMetricsEnabled) {
        recordScalar("num of frames sent successfully one retry",
                mpib.macRetryCount);
        recordScalar("num of frames sent successfully multi retry",
                mpib.macMultipleRetryCount);
        recordScalar("num of frames sent failed", mpib.macTXFailCount);
        recordScalar(
                "num of frames sent successfully within macAckWaitDuration",
                mpib.macTXSuccessCount);
        recordScalar("num of frames with FCS error", mpib.macFCSErrorCount);

        if (mpib.macSecurityEnabled)
            recordScalar("num of frames with security error",
                    mpib.macSecurityFailure);

        recordScalar("num of duplicated frames received",
                mpib.macDuplicateFrameCount);
        recordScalar("num of frames received successfully",
                mpib.macRXSuccessCount);
        recordScalar("num of ACK/NACK timing correction IE with NACK",
                mpib.macNACKcount);
    }
...
/**@author: 2014    Stefan Reis     (modified) */
void Ieee802154eMac::receiveChangeNotification(int category,
    ...
    switch (category) {
    if (check_and_cast<RadioState *>(details)->getRadioId()
            != getRadioModuleId())
        return;

case NF_RADIO_CHANNEL_CHANGED:
    ppib.phyCurrentChannel =
            check_and_cast<RadioState *>(details)->getChannelNumber();
    bitrate = getRate('b');
    phy_bitrate = bitrate;
...
case NF_RADIOSTATE_CHANGED:
    ...
    if (radioState == RadioState::RECV && mpib.macTSCHenabled) {
        if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
            ...
            EV
                      << "[TSCH CCA]-Transmitter:[10] start to receive a frame before TsAckWaitTimer timeout, cancel timeout timer"
                      << endl;
        ...
        } else if (taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA)
                && taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) == 4) {
            taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) = 5;
            EV
                      << "[TSCH CCA]-Receiver:[5] start to receive a frame before TsRxWaitTimer timeout, cancel timeout timer"
                      << endl;
...
case NF_PHY_PKT_DISCARDED:     // a frame is discarded at the PHY layer
    if (mpib.macTSCHenabled) { // we need this, to detect that the PHY layer finished the receiving of a frame
        if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
                && taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 10) {
            taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) = 11;
            EV
                      << "[TSCH CCA]-Transmitter:[11] frame was dropped at the PHY layer"
                      << endl;
            handleTsAckWait();
            ...
            EV << "[TSCH CCA]-Receiver:[6] frame was dropped at the PHY layer"
                      << endl;
            handleTsRxWait();
...
void Ieee802154eMac::handleMessage(cMessage* msg) {
    ...
    if (msg->getArrivalGateId()
            == mLowerLayerIn&& dynamic_cast<cPacket*>(msg) == NULL) {
        if (msg->getKind() == 0)
            error(
                    "[MAC]: message '%s' with length == 0 is supposed to be a primitive, but msg kind is also zero",
                    msg->getName());
    ...
    if (msg->getArrivalGateId()
            == mUpperLayerIn&& dynamic_cast<cPacket*>(msg) == NULL) {
        if (msg->getKind() == 0)
            error(
                    "[MAC]: message '%s' with length == 0 is supposed to be a primitive, but msg kind is also zero",
                    msg->getName());
...
bool Ieee802154eMac::handlePrimitive(int msgkind, cMessage *msg) // for MLME-SAP (Command messages)
        {
    Ieee802154eNetworkCtrlInfo *primitive = check_and_cast<
            Ieee802154eNetworkCtrlInfo *>(msg);
    switch (msgkind) {
...
//                UINT_64 lowLatencyNetworkInfo, UINT_16 channelOffset, UINT_8 hoppingSequenceID);
    ...
    case TP_MLME_GTS_REQUEST:
        ...
        handle_MLME_GTS_request(primitive->getGtsCharacteristic(),
                primitive->getSecurityLevel(), primitive->getKeyIdMode(),
                primitive->getKeySource(), primitive->getKeyIndex());
        delete primitive;
        break;
    ...
    case TP_MLME_RX_ENABLE_REQUEST:
        ...
        handle_MLME_RX_ENABLE_request(primitive->getDeferPermit(),
                primitive->getRxOnTime(), primitive->getRxOnDuration(),
    ...
    case TP_MLME_SYNC_REQUEST:
        ...
        handle_MLME_SYNC_request(primitive->getChannelNumber(),
                primitive->getChannelPage(), primitive->getTrackBeacon());
        delete primitive;
        break;

    case TP_MLME_POLL_REQUEST:
        EV << "[MAC]: this is a MLME_POLL_REQUEST" << endl;
        handle_MLME_POLL_request(
    ...
    case TP_MLME_DPS_REQUEST:
        ...
        handle_MLME_DPS_request(primitive->getTxDPSIndex(),
                primitive->getRxDPSIndex(), primitive->getDpsIndexDuration());
        delete primitive;
        break;
    ...
    case TP_MLME_CALIBRATE_REQUEST:
        EV
                  << "[MAC]: this is a MLME_CALIBRATE_REQUEST - Note: UWB is not supported"
                  << endl;
...
void Ieee802154eMac::handleUpperMsg(cPacket* msg) // MCPS-SAP
    ...
    if (taskP.taskStatus(TP_MCPS_DATA_REQUEST)) {
        EV << "[MAC]: an " << msg->getName() << " (#" << numUpperPkt
                  ...
                  << endl;
        MCPS_DATA_confirm(0, msg->getArrivalTime().getScale(), false, 0, 0, 0,
                0, 0, mac_TRANSACTION_EXPIRED, 0, 0, msg);
        delete msg;
        numUpperPktLost++;
        reqtMsgFromIFq();
        return;
    ...
    if (dynamic_cast<Ieee802Ctrl *>(controlInfo)) // for IPv4 Network layer and MANET routing
    {
        ...
        } else if (destAddr.equals(mpib.macCoordExtendedAddress)
                || destAddr.equals((IE3ADDR) mpib.macCoordShortAddress)) // packet destined for my coordinator
                        {
            if (notAssociated) {
                EV << "[MAC]: an " << msg->getName()
                          << " destined for the coordinator received from the upper layer, but drop it due to being not associated with any coordinator yet"
                          << endl;
                MCPS_DATA_confirm(0, msg->getArrivalTime().getScale(), false, 0,
                        0, 0, 0, 0, mac_INVALID_ADDRESS, 0, 0, msg);
                delete msg;
                delete cInfo;
                numUpperPktLost++;
                reqtMsgFromIFq();
                ...
                if (gtsStartSlot == 0 || isRecvGTS) // no transmit GTS
                        {
                    EV << "[MAC]: an " << msg->getName()
                              << " requesting GTS transmission destined for the PAN Coordinator received from the upper layer, but drop it due to no transmit GTS allocated by the PAN coordinator yet"
                              << endl;
                    MCPS_DATA_confirm(0, msg->getArrivalTime().getScale(),
                            false, 0, 0, 0, 0, 0, mac_INVALID_GTS, 0, 0, msg);
                    delete msg;
                    delete cInfo;
                    numUpperPktLost++;
                    reqtMsgFromIFq();
        ...
        } else        //  packet destined for device
        {
            ...
            if (mpib.macShortAddress != 0xfffe) // device is associated to a PAN coordinator
                ...
                if (mpib.macShortAddress != 0xfffe
                        && deviceList.find(getShortAddress(destAddr))
                                == deviceList.end()
                        && !neighborTable->isNeighborBySAddr(
                                getShortAddress(destAddr))) {
                    EV << "[MAC]: an " << msg->getName()
                              << " destined for the device with MAC address "
                              << destAddr
                              << " received from the upper layer, but drop it due to no device with this address found in my device list"
                              << endl;
                    MCPS_DATA_confirm(0, msg->getArrivalTime().getScale(),
                            false, 0, 0, 0, 0, 0, mac_INVALID_ADDRESS, 0, 0,
                            msg);
                    delete msg;
                    delete cInfo;
                    numUpperPktLost++;
                ...
                if (dataTransMode == 3)  // send in GTS (set in the omnetpp.ini)
                    ...
                    for (index = 0; index < gtsCount; index++) {
                        if (gtsList[index].devShortAddr
                                == getShortAddress(destAddr)
                                && gtsList[index].isRecvGTS) {
                            gtsFound = true;
                            break;
                        }
                    }
                    if (gtsFound) {
                        ...
                        EV << "[MAC]: an " << msg->getName()
                                  << " requesting GTS transmission destined for the device received from the upper layer, but drop it due to no valid GTS for this device found in my GTS list"
                                  << endl;
                        MCPS_DATA_confirm(0, msg->getArrivalTime().getScale(),
                                false, 0, 0, 0, 0, 0, mac_INVALID_GTS, 0, 0,
                                msg);
                        delete msg;
                        delete cInfo;
                        numUpperPktLost++;
                ...
                } else { // other transfer mode (dataTransMode -> direct or indirect) - send it in CAP
                    gtsTX = false;     // send in the CAP
    ...
    } else // for the StarRouting Network layer - for the StarNet and Nodes Random Direct example
    {
        ...
        Ieee802154eNetworkCtrlInfo* cInfo = check_and_cast<
                Ieee802154eNetworkCtrlInfo *>(controlInfo);
        ...
        } else        //  packet destined for device
        {
            if (!simulation.getModuleByPath(cInfo->getDestName()))
                error(
                        "[MAC]: address conversion fails, destination host does not exist!");

#if OMNETPP_VERSION >= 0x403
            cModule* module =
            simulation.getModuleByPath(cInfo->getDestName())->getModuleByPath(
                    ".wlan.mac");
#else
            cModule* module = simulation.getModuleByPath(cInfo->getDestName())->getModuleByRelativePath("wlan.mac");
#endif

            Ieee802154eMac* macModule = check_and_cast<Ieee802154eMac *>(
                    module);
            destAddr = macModule->getMacAddr();
            destPanId = mpib.macPANId;

            // check if dest node is in my device list (associated or not)
            if (deviceList.find(getShortAddress(destAddr))
                    == deviceList.end()) {
                EV << "[MAC]: an " << msg->getName()
                          << " destined for the device with MAC address "
                          << destAddr
            ...
            if (dataTransMode == 3) // Std 802.15.4-2011 (4.5.2) page 13 - Note: The third transaction is the data transfer between two peer devices.
                ...
                // check if there is a receive GTS allocated for the dest node in my GTS list
                for (index = 0; index < gtsCount; index++) {
                    if (gtsList[index].devShortAddr == getShortAddress(destAddr)
                            && gtsList[index].isRecvGTS) {
                        gtsFound = true;
                        break;
                    }
    ...
    EV << "[MAC]: an " << msg->getName() << " (#" << numUpperPkt << ", "
              << PK(msg)->getByteLength() << " Bytes, destined for MAC address " << destAddr << ", transfer mode " << dataTransMode << ") received from the upper layer" << endl;
    ...
    frameControlOptions.iesIncluded = (useTSCH) ? true : false;
    frameControlOptions.seqSuppressed = false;

    std::vector<Ieee802154eIEHeaderType> headerIElist(
            Ieee802154eIEHeaderType(0x1a));
    std::vector<Ieee802154eIEPayloadType> payloadIElist(
            Ieee802154eIEHeaderType(0x00));
    ...
    MCPS_DATA_request((Ieee802154eAddrMode) srcAddrMode,
            (Ieee802154eAddrMode) (
                    destAddr.getFlagEui64() ?
                    defFrmCtrl_AddrMode64 :
                                              defFrmCtrl_AddrMode16), destPanId,
            destAddr, (UINT_8) msg->getByteLength(), msg,
            mpib.macDSN++/*msduHandle */,
            ackTX, gtsTX, indirectTX, secuData/*securityLevel*/, 0/*keyIdMode*/,
            0/*keySource*/, 0/*keyIndex*/, UWBPRF_PRF_OFF, RANGING_OFF,
            0/*uwbPreambleSymbolRepetitions,*/, 0/*dataRate*/,
            frameControlOptions, headerIElist, payloadIElist,
...
void Ieee802154eMac::handleLowerMsg(cPacket* msg) // PD_SAP
    ...
    if (!frame) {
        EV << "[MAC]: message from physical layer (" << msg->getClassName()
                  << ")" << msg->getName()
                  << " is not a subclass of Ieee802154eFrame, drop it" << endl;
        delete frame;
        return;
    }
    ...
    EV << "[MAC]: an " << frameTypeTxt[frmType]
    ...
    if (frameFilter(frame)) {
        ...
        if (mpib.macTSCHenabled) {
            if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
                ...
                taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) = 11;
                EV
                          << "[TSCH CCA]-Transmitter:[11] frame was dropped at the MAC layer"
                          << endl;
                handleTsAckWait();
                // next step in handleTsAckWait()
            } else if (taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA)
                ...
                taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) = 6;
                EV
                          << "[TSCH CCA]-Receiver:[6] frame was dropped at the MAC layer"
                          << endl;
                handleTsRxWait();
            }
        }
...
//    }
    ...
    if ((frmType == Ieee802154e_DATA) || (frmType == Ieee802154e_CMD)
            || (frmType == Ieee802154e_MULTI)
            || (frmType == Ieee802154e_ASSOCIATION_REQUEST)
            || (frmType == Ieee802154e_DISASSOCIATION_REQUEST)
            || (frmType == Ieee802154e_ASSOCIATION_RESPONSE)
            || (frmType == Ieee802154e_DISASSOCIATION_RESPONSE)
            || (frmType == Ieee802154e_SCHEDULER_REQUEST)
        ...
        if (frmCtrl.ackReq) //Acknowledgment required
        {
            ...
            if (!noAck) {
                ...
                if (mpib.macTSCHenabled
                        && taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA)
                        && taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) == 5) {
                    taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) = 7;
                    EV
                              << "[TSCH CCA]-Receiver:[7] the data frame arrived, set the TRX_STATE on TRX_OFF and wait the TsTxAckDelay"
                              << endl;
                    ...
                    scheduleAt(
                            simTime()
                                    + timeslotTable->getTemplate(useTimeslotID)->getTxAckDelayDbl(),
                            tsTxAckDelayTimer);
                    ...
                    if (backoffStatus == 99) {
                        EV
                                  << "[MAC]: CSMA-CA is pending, stop it, it will resume after sending ACK"
                                  << endl;
                        backoffStatus = 0;
                        csmacaCancel();
                    }
                    EV
                              << "[MAC]: prepare to send the ACK, ask PHY layer to turn on the transmitter first"
                              << endl;
                    PLME_SET_TRX_STATE_request(phy_TX_ON);
                }
            }
...
//            {   // if this is a broadcast or a beacon, go back to sleep
            if (taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA)
                    && taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) == 5) {
...
//            }
    ...
    if (frmType == Ieee802154e_CMD) {
        if ((rxCmd) || (txBcnCmd)) {
            EV
                      << "[MAC]: the received CMD frame is dropped, because MAC is currently processing a MAC CMD"
                      << endl;
            delete frame;
            return;
        }
    ...
    if (frmType == Ieee802154e_DATA) {
        if (rxData) {
            EV
                      << "[MAC]: the received DATA frame is dropped, because MAC is currently processing the last received DATA frame"
                      << endl;
            delete frame;
            return;
        }
    ...
    if (frmType == Ieee802154e_BEACON)
        i = chkAddUpdHListLink(&hlistBLink1, &hlistBLink2, frame->getSrcAddr(),
                frame->getSeqNmbr());
    else if (frmType != Ieee802154e_ACK) // data or cmd
        i = chkAddUpdHListLink(&hlistDLink1, &hlistDLink2, frame->getSrcAddr(),
                frame->getSeqNmbr());
    else // ACK
    {
...
//	    break;
    ...
    case Ieee802154e_ASSOCIATION_REQUEST:
        EV << "[MAC} continue to proxess received association request pkt"
                  << endl;
        MLME_ASSOCIATE_indication(frame);
        break;
    case Ieee802154e_ASSOCIATION_RESPONSE:
        EV << "[MAC} continue to process received association response pkt"
                  << endl;
...
void Ieee802154eMac::sendDown(Ieee802154eFrame* frame) {
    ...
    EV << "[MAC]: sending frame "
              << (frame->getFrmCtrl().frameType != Ieee802154e_DATA ?
                      (frame->getName()) :
                      (frame->getEncapsulatedPacket()->getName())) << " ("
              << frame->getByteLength() << " Bytes) to PHY layer" << endl;
    EV << "[MAC]: the estimated transmission time is " << calDuration(frame)
              << " s" << endl;
...
void Ieee802154eMac::handleBeacon(Ieee802154eFrame* frame) {
    if (frame->getFrmCtrl().frameVersion == Ieee802154_compatible
            && mpib.macTSCHcapable) // enhanced beacon frame, only if the device is TSCH capable
            {
        if (isSCAN) {
            ...
            if (isPANCoor
                    && !neighborTable->getNeighborByEAddr(frame->getSrcAddr())->isPrevStageCH()
                    && !notAssociated) {
                MLME_BEACON_NOTIFY_indication(frame);
            } else {
                rxBeacon = check_and_cast<Ieee802154EnhancedBeaconFrame *>(
                        frame);
                frame = NULL;
                Ieee802154eNetworkCtrlInfo *tmp =
                        new Ieee802154eNetworkCtrlInfo("BeaconConfirm",
...
void Ieee802154eMac::handleEB(cMessage *msg) {
    ...
    EV << "The first bit of this beacon was received by PHY layer at "
              << bcnRxTime << endl;
    ...
    rxPanDescriptor.coordAddrMode =
            (Ieee802154eAddrMode) rxBeacon->getFrmCtrl().srcAddrMode;
    ...
    if (firstBe) {
        ...
        int asnSlots =
                (timeCorr
                        / timeslotTable->getTemplate(useTimeslotID)->getTimeslotLengthDbl())
                        + 1;
    ...
    if (firstBe) {
        double temp =
                bcnRxTime.dbl()
                        - (timeslotTable->getTemplate(useTimeslotID)->getRxOffsetDbl()
                                + (timeslotTable->getTemplate(useTimeslotID)->getRxWaitDbl()
                                        / 2)) + timeCorr;
        int tempi =
                temp
                        / timeslotTable->getTemplate(useTimeslotID)->getTimeslotLengthDbl();
...
void Ieee802154eMac::handleData(Ieee802154eFrame* frame) {
    ...
    if (mpib.macTSCHenabled && frmCtrl.frameVersion == Ieee802154_compatible) // 802.15.4e Data frame
        ...
        MCPS_DATA_indication(
                (Ieee802154eAddrMode) rxData->getFrmCtrl().srcAddrMode,
                rxData->getSrcPanId(), rxData->getSrcAddr(),
                (Ieee802154eAddrMode) rxData->getFrmCtrl().dstAddrMode,
                rxData->getDstPanId(), rxData->getDstAddr(),
                rxData->getByteLength(), rxData, 0 /*mpduLinkQuality*/,
                rxData->getSeqNmbr(), (UINT_32) rxData->getTimestamp().raw(),
                rxData->getAuxSecHd().secLvl,
                rxData->getAuxSecHd().keyIdentMode,
                rxData->getAuxSecHd().keySource, rxData->getAuxSecHd().keyIndex,
                0 /*uwbPRF*/,
                (Ieee802154eUWBFType) 0 /*uwbPreambleSymbolRepetitions*/,
                0 /*dataRate*/, (RangingControl) 0 /*rangingReceived*/,
                0 /*rangingCounterStart*/, 0 /*rangingCounterStop*/,
                0 /*rangingTrackingInterval*/, 0 /*rangingOffset*/,
                0 /*rangingFOM*/);
    ...
    } else    // 802.15.4 Data frame
    {
        if (mpib.macTSCHenabled) {
            ...
            MCPS_DATA_indication(
                    ...
                    rxData->getByteLength(), rxData, 0 /*mpduLinkQuality*/,
                    rxData->getSeqNmbr(),
                    (UINT_32) rxData->getTimestamp().raw(),
                    rxData->getAuxSecHd().secLvl,
                    rxData->getAuxSecHd().keyIdentMode,
                    rxData->getAuxSecHd().keySource,
                    rxData->getAuxSecHd().keyIndex, 0 /*uwbPRF*/,
                    (Ieee802154eUWBFType) 0 /*uwbPreambleSymbolRepetitions*/,
                    0 /*dataRate*/, (RangingControl) 0 /*rangingReceived*/,
                    0 /*rangingCounterStart*/, 0 /*rangingCounterStop*/,
                    0 /*rangingTrackingInterval*/, 0 /*rangingOffset*/,
...
/**@author: 2014    Stefan Reis     (modified) */
void Ieee802154eMac::handleAck(Ieee802154eFrame* frame) //
        {
    if (frame->getFrmCtrl().frameType == Ieee802154e_ACK) {
        if (frame->getFrmCtrl().frameVersion == Ieee802154_compatible) // 802.15.4e Enhanced ACK frame
            ...
            if (mpib.macTSCHenabled && taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
                    && taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 10) {
                ...
                if (frame->getSeqNmbr() != txData->getSeqNmbr()) {
                    delete frame;
                    EV
                              << "[TSCH CCA]-Transmitter:[11] frame was dropped at the MAC layer, the SeqNumber in the ACK does not match"
                              << endl;
...
//			int test = 0;
                    ...
                    int timeslot =
                            FWMath::modulo(curASN,
                                    slotframeTable->getSlotframe(
                                            activeLinkEntry->getSlotframeId())->getSlotframeSize());
                    ...
                    string tmpTxt =
                            (txData->getFrmCtrl().frameType == Ieee802154e_DATA) ?
                                    txData->getEncapsulatedPacket()->getName() :
                                    frameTypeTxt[txData->getFrmCtrl().frameType];
                    EV
                              << "[TSCH CCA]-Transmitter:[10] the correct ACK arrives, transmission of the "
                              << txPkt->getName() << "(" << tmpTxt
                              << ") frame was successful, delete frame from queue"
                              << endl;

                    MCPS_DATA_confirm(0, txData->getArrivalTime().getScale(),
                            false, 0, 0, 0, 0, 0, mac_SUCCESS,
                            txData->getRetries(), 0, txData);
                    ...
                    if (destAddr != 0xffff) // if is not the broadcast address
                            {
                        activeNeighborEntry = neighborTable->getNeighborBySAddr(
                                destAddr);
                    ...
                    if (txPkt == txData) {
                        if (txData->getFrmCtrl().frameType
                                == Ieee802154e_DISASSOCIATION_RESPONSE) {
                            neighborTable->deleteNeighbor(
                                    neighborTable->getNeighborBySAddr(
                                            getShortAddress(
                                                    txData->getDstAddr())));
                        }
                        delete txData;
        ...
        {   //[SR] old version
            if ((txBcnCmd == NULL) && (txBcnCmdUpper == NULL)
                    && (txData == NULL) && (txGTS == NULL)) {
                EV
                          << "[MAC]: no pending transmission task is waiting for this ACK, drop it!"
                          << endl;
                delete frame;
                return;
            }

            if ((txPkt != txBcnCmd) && (txPkt != txBcnCmdUpper)
                    && (txPkt != txData) && (txPkt != txGTS))
                    // ack received after corresponding task has failed duo to reaching max retries
                    {
                EV
                          << "[MAC]: this is a late ACK received after corresponding task has failed, drop it!"
                          << endl;
                delete frame;
                return;
            }
            ...
            //check the sequence number in the ACK to see if it matches that in the <txPkt>
            if (frame->getSeqNmbr()
                    != check_and_cast<Ieee802154eFrame *>(txPkt)->getSeqNmbr()) {
                EV << "[MAC]: the SeqNumber in the ACK does not match, drop it!"
                          << endl;
                delete frame;
                return;
            }
            ...
            if (ackTimeoutTimer->isScheduled()) // ACK arrives before ACK timeout expires
            {
                numRxAckPkt++;
                EV
                          << "[MAC]: the ACK arrives before timeout, cancel timeout timer"
                          << endl;
                ...
                if (txPkt != txData) {
                    EV
                              << "[MAC]: this is a late ACK, but not for a DATA pkt, drop it!"
                              << endl;
                    delete frame;
                    return;
                }
                ...
                if (backoffStatus == 99) {
                    EV << "[MAC]: this is a late ACK for " << txData->getName()
                              << ":#" << (int) txData->getSeqNmbr()
                              << ", stop retrying" << endl;
                    backoffStatus = 0;
                    csmacaCancel();
                }
...
/**@author: 2014    Stefan Reis     (modified) */
//void Ieee802154eMac::handleCommand80215(Ieee802154eFrame* frame) // : include the 802.15.4e Command Frame [SR]
//{
...
//}
/**@author: 2014    Stefan Reis */
void Ieee802154eMac::handleLLDN802154e(Ieee802154eFrame*) //
        {

}

/**@author: 2014    Stefan Reis */
void Ieee802154eMac::handleMulti802154e(Ieee802154eFrame*)
        {
...
bool Ieee802154eMac::frameFilter(Ieee802154eFrame* frame) {
    ...
    if (!mpib.macPromiscuousMode) {
        //check packet type
        if ((frmType != Ieee802154e_BEACON) && (frmType != Ieee802154e_DATA)
                && (frmType != Ieee802154e_ACK) && (frmType != Ieee802154e_CMD)
                && (frmType != Ieee802154e_LLDN)
                && (frmType != Ieee802154e_MULTI)
                && (frmType != Ieee802154e_ASSOCIATION_REQUEST)
                && (frmType != Ieee802154e_DISASSOCIATION_REQUEST)
                && (frmType != Ieee802154e_DISASSOCIATION_RESPONSE)
        ...
        if (frmCtrl.frameVersion == Ieee802154_compatible) {
            ...
            if (frame->getSrcPanId() == mpib.macPANId
                    || frmType == Ieee802154e_BEACON) {
        ...
        } else {   // for Std 802.15.4-2006 & Std 802.15.4-2011
            ...
            {   // is not my or the broadcast address
                if (frame->getDstAddr() != mpib.macExtendedAddress
                        && !frame->getDstAddr().isBroadcast()) {
                    return true;
                }
            }
...
void Ieee802154eMac::constructACK(Ieee802154eFrame* rxFrame) {
    ...
    if (mpib.macTSCHenabled) { // enhanced Acknowledgment - see IEEE Std 802.15.4e-2012 (5.2.2.3 Acknowledgment frame format) page 66
        ...
        Ieee802154eEnhancedAckFrame* tmpEAck =
                new Ieee802154eEnhancedAckFrame();
        ...
        if (ackFrmCtrl.dstAddrMode == defFrmCtrl_AddrModeNone) { // no address
            tmpDstPanId = 0;
            tmpDstAddr = (IE3ADDR) 0xffff;
        } else if (ackFrmCtrl.dstAddrMode == defFrmCtrl_AddrMode8) { // 8 bit address
            tmpDstPanId = rxFrame->getSrcPanId();
            tmpDstAddr = IE3ADDR((UINT_8) rxFrame->getSrcAddr().getInt());
        } else if (ackFrmCtrl.dstAddrMode == defFrmCtrl_AddrMode16) { // 16 bit address
            tmpDstPanId = rxFrame->getSrcPanId();
        ...
        if (ackFrmCtrl.srcAddrMode == defFrmCtrl_AddrModeNone) { // no address
            tmpSrcPanId = 0;
            tmpSrcAddr = (IE3ADDR) 0xffff;
        } else if (ackFrmCtrl.srcAddrMode == defFrmCtrl_AddrMode8) { // 8 bit address
            tmpSrcPanId = mpib.macPANId;
            tmpSrcAddr = (IE3ADDR) mpib.macSimpleAddress;
        } else if (ackFrmCtrl.srcAddrMode == defFrmCtrl_AddrMode16) { // 16 bit address
            tmpSrcPanId = mpib.macPANId;
...
void Ieee802154eMac::constructBCN() {
    ...
    if (mpib.macTSCHenabled && mpib.macUseEnhancedBeacon) {
        ...
        Ieee802154EnhancedBeaconFrame* tmpEBcn =
                new Ieee802154EnhancedBeaconFrame();
        tmpEBcn->setName("Ieee802154BEACON");
        tmpEBcn->setBO(mpib.macBeaconOrder);
        tmpEBcn->setSO(mpib.macSuperframeOrder);
        ...
        //
        if (frmCtrl.dstAddrMode == defFrmCtrl_AddrModeNone) { // no address
            tmpDstPanId = 0;
            tmpDstAddr = (IE3ADDR) 0xffff;
        } else if (frmCtrl.dstAddrMode == defFrmCtrl_AddrMode8) { // 8 bit address
            tmpDstPanId = 0;
            tmpDstAddr = (IE3ADDR) 0xffff;
        } else if (frmCtrl.dstAddrMode == defFrmCtrl_AddrMode16) { // 16 bit address
            tmpDstPanId = 0;
            tmpDstAddr = (IE3ADDR) 0xffff;
        } else { // 64 bit address
            tmpDstPanId = 0;
        ...
        if (frmCtrl.srcAddrMode == defFrmCtrl_AddrModeNone) { // no address
            tmpSrcPanId = 0;
            tmpSrcAddr = (IE3ADDR) 0xffff;
        } else if (frmCtrl.srcAddrMode == defFrmCtrl_AddrMode8) { // 8 bit address
            tmpSrcPanId = mpib.macPANId;
            tmpSrcAddr = (IE3ADDR) mpib.macSimpleAddress;
        } else if (frmCtrl.srcAddrMode == defFrmCtrl_AddrMode16) { // 16 bit address
            tmpSrcPanId = mpib.macPANId;
            tmpSrcAddr = (IE3ADDR) mpib.macShortAddress;
        } else { // 64 bit address
            tmpSrcPanId = mpib.macPANId;
        ...
        frmCtrl.compPanID = getPANIDComp(frmCtrl, tmpEBcn->getSrcPanId(),
                tmpEBcn->getDstPanId());
        ...
        SlotframeLinkIE* tmpTSCHSlot = new SlotframeLinkIE(slotframeTable,
                linkTable); // Std 802.15.4e-2012 (5.2.4.13 TSCH synchronization IE) page 90
        tmpSlotIE->ieNestedMLMELength = tmpTSCHSlot->fillSlotframeLinkIE();
        tmpSlotIE->ieTSCHSLotfrmLink = tmpTSCHSlot;

        if ((lengthNoIE + payloadLength + tmpSlotIE->ieNestedMLMELength + 3)
                < aMaxPHYPacketSize) {
        ...
        if ((lengthNoIE + payloadLength + 3) < aMaxPHYPacketSize) {
            ...
            bool tmpAll =
                    ((lengthNoIE + payloadLength + 25 + 2) < aMaxPHYPacketSize) ?
                            true : false;
            ...
            if (tmpAll) {
                ...
                tmpTSCHTimeSlt->setRxAckDelay(
                        timeslotTemplate->getRxAckDelay());
                tmpTSCHTimeSlt->setTxAckDelay(
                        timeslotTemplate->getTxAckDelay());
                ...
                tmpTSCHTimeSlt->setTimeslotLength(
                        timeslotTemplate->getTimeslotLength());
            }
            ...
            if (tmpAll) {
                ...
                tmpChannel->channelPage =
                        hoppingSequenceList->getEntry(0)->getChannelPage();
                tmpChannel->currentHop =
                        hoppingSequenceList->getEntry(0)->getCurrentHop();
                tmpChannel->extBitmap =
                        hoppingSequenceList->getEntry(0)->getExtendedBitmap();
                tmpChannel->hoppingSeqID =
                        hoppingSequenceList->getEntry(0)->getHoppingSequenceId();
                tmpChannel->hoppingSeqLength =
                        hoppingSequenceList->getEntry(0)->getHoppingSequenceLength();
                tmpChannel->hoppingSeqLst =
                        hoppingSequenceList->getEntry(0)->getHoppingSequenceList();
                tmpChannel->numberOfChannels =
                        hoppingSequenceList->getEntry(0)->getNumberOfChannels();
                tmpChannel->phyConfig =
                        hoppingSequenceList->getEntry(0)->getPhyConfiguration();
        ...
        frmCtrl.presIElist =
                ((lengthNoIE + payloadLength) <= aMaxPHYPacketSize) ?
                        true : false;
        ...
        if (mpib.macTSCHenabled) { // save in the mac queue
...
void Ieee802154eMac::resetTRX() {
    ...
    if (mpib.macTSCHenabled) {
        EV
                  << "[MAC]: it's now in inactive period, should turn off radio and go to sleep"
                  << endl;
        ...
        if ((mpib.macBeaconOrder != 15) || (rxBO != 15))  // beacon enabled
            ...
            {
                EV
                          << "[MAC]: it's now in outgoing active period (as a coordinator), should stay awake and turn on receiver"
                          << endl;
                t_state = phy_RX_ON;
            } else                     // in rx SD, according to macRxOnWhenIdle
            {
                EV
                          << "[MAC]: it's now in incoming active period (as a device), whether go to sleep depending on parameter macRxOnWhenIdle"
                          << endl;
                t_state = mpib.macRxOnWhenIdle ? phy_RX_ON : phy_TRX_OFF;
            }
        } else // non-beacon
        {
            EV
                      << "[MAC]: non-beacon, whether go to sleep depending on parameter macRxOnWhenIdle"
                      << endl;
            t_state = mpib.macRxOnWhenIdle ? phy_RX_ON : phy_TRX_OFF;
        }
    }
...
void Ieee802154eMac::PD_DATA_request(UINT_8 psduLength,
        Ieee802154eFrame * psdu) {
...
void Ieee802154eMac::handle_PD_DATA_indication(UINT_8 psduLength, cPacket* psdu,
...
void Ieee802154eMac::PLME_SET_TRX_STATE_request(PHYenum state) {
    ...
    if (!trx_state_req_activ && trx_state_req != state) {
        EV << "[MAC]: sending PLME-SET-TRX-STATE.request <" << stateTxt[state]
                  << "> to PHY layer" << endl;

        // construct PLME_SET_TRX_STATE_request primitive
        Ieee802154eMacPhyPrimitives *primitive =
                new Ieee802154eMacPhyPrimitives();
...
void Ieee802154eMac::PLME_SET_request(PHYPIBenum pibAttribute,
...
void Ieee802154eMac::handleMacPhyPrimitive(int msgkind, cMessage* msg) {
    Ieee802154eMacPhyPrimitives* primitive = check_and_cast<
            Ieee802154eMacPhyPrimitives *>(msg);
    switch (msgkind) {
...
case PLME_ED_CONFIRM:
    handle_PLME_ED_confirm(PHYenum(primitive->getStatus()),
            primitive->getEnergyLevel());
    delete primitive;
    break;

case PLME_GET_CONFIRM:
    handle_PLME_GET_confirm(PHYenum(primitive->getStatus()),
            PHYPIBenum(primitive->getPibAttribute()),
            PHY_PIB(primitive->getPibAttributeValue()));
    delete primitive;
    break;
...
case PLME_SET_CONFIRM:
    handle_PLME_SET_confirm(PHYenum(primitive->getStatus()),
            PHYPIBenum(primitive->getPibAttribute()));
    delete primitive;
    break;
    ...
    if (mpib.macTSCHenabled) {
        if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
            ...
            if (status == phy_SUCCESS) // successful transmission
                    {
                Ieee802154eFrame* tmpPkt = check_and_cast<Ieee802154eFrame *>(
                        txPkt);
                if (tmpPkt->getFrmCtrl().ackReq) {   // Acknowledge required
                    taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) = 7;
                    EV << "[TSCH CCA]-Transmitter:[7] wait the TsRxAckDelay"
                              << endl;
                    ...
                    scheduleAt(
                            simTime()
                                    + timeslotTable->getTemplate(useTimeslotID)->getRxAckDelayDbl(),
                            tsRxAckDelayTimer);

                    PLME_SET_TRX_STATE_request(phy_TRX_OFF);
                    // next step at handleTsRxAckDelayTimer()
                } else {   // no Acknowledge required
                    taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA) = false;
                    taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) = 0;
                    EV
                              << "[TSCH CCA]-Transmitter:[6] frame without Acknowledge required successful transmitted go back to sleep"
                              << endl;

                    if (tsRxAckDelayTimer->isScheduled())
                        cancelEvent(tsRxAckDelayTimer);

                    MCPS_DATA_confirm(0, tmpPkt->getArrivalTime().getScale(),
                            false, 0, 0, 0, 0, 0, mac_SUCCESS,
                            tmpPkt->getRetries(), 0, tmpPkt);
            ...
            if (status == phy_SUCCESS)  // transmission successful
                EV
                          << "[TSCH CCA]-Receiver:[10] successful transmitting of the ACK frame in the TsMacAckTime, go back to sleep"
                          << endl;
            else
                // transmission failed
                EV
                          << "[TSCH CCA]-Receiver:[10] failed transmission of the ACK frame in the tsMaxAckTime"
                          << endl;
...
void Ieee802154eMac::handle_PLME_CCA_confirm(PHYenum status) {
    if (mpib.macTSCHenabled && taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA) == true) {
        if (status == phy_IDLE && taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 4) {
            ...
            EV << "[TSCH CCA]-Transmitter:[5] switch from Rx to Tx state"
                      << endl;
            PLME_SET_TRX_STATE_request(phy_TX_ON);
...
void Ieee802154eMac::handle_PLME_ED_confirm(PHYenum status,
        UINT_8 energyLevel) {
...
void Ieee802154eMac::handle_PLME_GET_confirm(PHYenum status,
        PHYPIBenum pibAttribute, PHY_PIB pibAttributeValue) {
    ...
    if (status == phy_SUCCESS) {
        ...
        case PHY_PREAMBLE_SYMBOL_LENGTH: {
            ppib.phyPreambleSymbolLength =
                    pibAttributeValue.phyPreambleSymbolLength;
            break;
        }
        case PHY_UWB_DATA_RATES_SUPPORTED: {
            ppib.phyUWBDataRatesSupported =
                    pibAttributeValue.phyUWBDataRatesSupported;
            break;
        }
        case PHY_CSS_LOW_DATA_RATE_SUPPORTED: {
            ppib.phyCSSLowDataRateSupported =
                    pibAttributeValue.phyCSSLowDataRateSupported;
            break;
        }
        case PHY_UWB_COU_SUPPORTED: {
        ...
        case PHY_UWB_CURRENT_PULSE_SHAPE: {
            ppib.phyUWBCurrentPulseShape =
                    pibAttributeValue.phyUWBCurrentPulseShape;
            break;
        }
        case PHY_UWB_COU_PULSE: {
        ...
        case PHY_RANGING_CRYSTAL_OFFSET: {
            ppib.phyRangingCrystalOffset =
                    pibAttributeValue.phyRangingCrystalOffset;
            break;
        }
        case PHY_RANGING_DPS: {
        ...
        case PHY_UWB_SCAN_BINS_PER_CHANNEL: {
            ppib.phyUWBScanBinsPerChannel =
                    pibAttributeValue.phyUWBScanBinsPerChannel;
            break;
        }
        case PHY_UWB_INSERTED_PREAMBLE_INTERVAL: {
            ppib.phyUWBInsertedPreambleInterval =
                    pibAttributeValue.phyUWBInsertedPreambleInterval;
            break;
        }
        case PHY_TX_RMARKER_OFFSET: {
        ...
        case PHY_RFRAME_PROCESSING_TIME: {
            ppib.phyRFRAMEProcessingTime =
                    pibAttributeValue.phyRFRAMEProcessingTime;
            break;
        }
        case PHY_CCA_DURATION: {
...
void Ieee802154eMac::handle_PLME_SET_TRX_STATE_confirm(PHYenum status) {
    EV << "[MAC]: a PLME_SET_TRX_STATE_confirm with " << stateTxt[status]
              << " received from PHY, the requested state is "
              << stateTxt[trx_state_req] << endl;
    ...
    if (status == trx_state_req) {
        ...
        trx_state_req_activ = false;
    } else { // to cancelled the msg, if we switch from Rx to Tx state or reverse
        trx_state_req_activ = false;
        return;
    }
        ...
        if (trx_state_req == phy_RX_ON) {
            ...
            if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)) {
                if (taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 3) {
                    ...
                    taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) = 9;
                    EV << "[TSCH CCA]-Transmitter:[9] start the TsAckWaitTimer"
                              << endl;
                    ...
                    scheduleAt(
                            simTime()
                                    + timeslotTable->getTemplate(useTimeslotID)->getAckWaitDbl(),
                            tsAckWaitTimer);
                    return;
                    /* two possible next steps:
                    ...
                    && taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) == 3) // TSCH CCA Receiver
                            {
                taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) = 4;
            ...
            if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
                ...
                // next step will be the PD_DATA_confirm()
                    ...
                    && taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) == 8) // TSCH CCA Receiver
                            {
                taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) = 9;
                ...
                if (calDuration(txAck)
                        <= timeslotTable->getTemplate(useTimeslotID)->getMaxAckDbl()) {
                    txPkt = txAck;
                    PD_DATA_request(txAck->getByteLength(),
                            check_and_cast<Ieee802154eFrame *>(txAck->dup()));
                    return;
                    // next step will be the PD_DATA_confirm()
                } else {
                    EV
                              << "[TSCH CCA]-Receiver:[9-ERROR]: the transmission duration ("
                              << calDuration(txAck)
                              << " s ) is bigger than the macTsMaxAck ("
                              << timeslotTable->getTemplate(useTimeslotID)->getMaxAckDbl()
                                      * 1000 << " ms)" << endl;
...
//             delay = 0.0;
        else                                //beacon enabled
        {
            ...
            delay = csmacaLocateBoundary(
                    (getShortAddress(txAck->getDstAddr())
                            == mpib.macCoordShortAddress), 0.0);
        ...
        txPkt = txGTS;
        PD_DATA_request(txGTS->getByteLength(),
                check_and_cast<Ieee802154eFrame *>(txGTS->dup()));
    } else        // tx cmd or data
    {
        if ((mpib.macBeaconOrder == 15) && (rxBO == 15))  //non-beacon enabled
            delay = 0.0;
...
void Ieee802154eMac::handle_PLME_SET_confirm(PHYenum status,
        PHYPIBenum pibAttribute) {
    EV << "[MAC]: a PLME_SET_confirm with " << stateTxt[status]
              << " received from PHY, the requested PIBAttribute is "
              << paraTxt[pibAttribute] << endl;
    ...
    if (status == phy_SUCCESS && pibAttribute == ppib_pibAttribure_req) {
        ...
        if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)) {
            if (taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 2) {
                ...
                EV
                          << "[TSCH CCA]-Transmitter:[3] before sending CCA primitive, tell the PHY layer to turn on the receiver"
                          << endl;
                PLME_SET_TRX_STATE_request(phy_RX_ON);
                // next step in handle_PLME_SET_TRX_STATE_confirm()
                ...
                taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) = 5;
                EV
                          << "[TSCH CCA]-Transmitter:[5] (without CCA) switch to Tx state"
                          << endl;
                PLME_SET_TRX_STATE_request(phy_TX_ON);
                // next step in handle_PLME_SET_TRX_STATE_confirm()
            }
            ...
            EV
                      << "[TSCH CCA]-Receiver:[3] set the TRX_STATE to RX_ON and wait for the pkt"
                      << endl;
            PLME_SET_TRX_STATE_request(phy_RX_ON);
 ...
 *
 * param[in]     "param1 Input parameter (i.e. read by this function). */
void Ieee802154eMac::MCPS_DATA_request(Ieee802154eAddrMode srcAddrMode,
        Ieee802154eAddrMode dstAddrMode, UINT_16 dstPANId, IE3ADDR dstAddr,
        UINT_8 msduLength, cPacket* msdu, UINT_8 msduHandle, bool ackTX,
        bool gtsTX, bool indirectTX, UINT_8 securityLevel, UINT_8 keyIdMode,
        UINT_64 keySource, UINT_8 keyIndex, Ieee802154eUWBFType uwbPRF,
        RangingControl ranging, UINT_16 uwbPreambleSymbolRepetitions,
        UINT_8 dataRate, FrameCrlOptions frameControlOptions,
        std::vector<Ieee802154eIEHeaderType> headerIElist,
        std::vector<Ieee802154eIEPayloadType> payloadIElist,
    ...
    if (sendMultipurpose && mpfrmCtrl.lngFrameCtrl) {
        ...
        frmCtrl.securityEnabled =
                (securityLevel == ASH_SLVL_NONE) ? false : true; //by default: false
        ...
        frmCtrl.securityEnabled =
                (securityLevel == ASH_SLVL_NONE) ? false : true; //secuData;
        frmCtrl.frmPending = false;
        frmCtrl.ackReq = ackTX;

        // see Std 802.15.4-2011 page 118 or see Std 802.15.4e-2012 (5.2.3 Frame compatibility) page 79
        if (msduLength > aMaxMACSafePayloadSize
                || msdu->getByteLength() > aMaxMACSafePayloadSize)
        ...
        if (frmCtrl.frameVersion == Ieee802154_2003_compatible
                || frmCtrl.frameVersion == Ieee802154_2006_introduced) {
    ...
    if (sendMultipurpose) {
        if (mpfrmCtrl.presPANID) { // see Std 802.15.4e-2012 (5.2.2.6.13) page77
    ...
    if (sendMultipurpose) {
        frmCtrl.compPanID = getPANIDComp(frmCtrl, tmpMPData->getSrcPanId(),
                tmpMPData->getDstPanId()); // PAN ID Compression field
//        tmpMPData->setIsGTS(gtsTX);
        ...
        EV << "[MAC]: MPDU constructed: " << tmpMPData->getName() << " ("
                  << tmpMPData->getEncapsulatedPacket()->getName() << "), #"
                  << (int) tmpMPData->getSeqNmbr() << ", "
                  << tmpMPData->getByteLength() << " Bytes" << endl;
    } else {
        frmCtrl.compPanID = getPANIDComp(frmCtrl, tmpData->getSrcPanId(),
                tmpData->getDstPanId()); // PAN ID Compression
        ...
        EV << "[MAC]: MPDU constructed: " << tmpData->getName() << " ("
                  << tmpData->getEncapsulatedPacket()->getName() << "), #"
                  << (int) tmpData->getSeqNmbr() << ", "
                  << tmpData->getByteLength() << " Bytes" << endl;
    }
    ...
    case DIRECT_TRANS: // send in the CAP or at the Link for TSCH transmission
    {
        ...
        if (mpib.macTSCHenabled) {
            ...
            if (duration
                    > timeslotTable->getTemplate(useTimeslotID)->getMaxTxDbl()) // delete msg and report MLME_DATA_confirm
                    {
                EV << "[TSCH CCA]: the estimated transmission time ("
                          << duration << " s) is bigger then "
                          << timeslotTable->getTemplate(useTimeslotID)->getMaxTxDbl()
                                  * 1000 << " ms, msg will be discarded"
                          << endl;

                if (sendMultipurpose)
                    MCPS_DATA_confirm(tmpMPData->getSeqNmbr(), 0, false, 0, 0,
                            0, 0, 0, mac_FRAME_TOO_LONG, 0, 0, tmpMPData);
                else
                    MCPS_DATA_confirm(tmpData->getSeqNmbr(), 0, false, 0, 0, 0,
                            0, 0, mac_FRAME_TOO_LONG, 0, 0, tmpData);
                ...
                EV << "[TSCH CCA]: the estimated transmission time ("
                          << duration << " s) is smaller or equal as the "
                          << timeslotTable->getTemplate(useTimeslotID)->getMaxTxDbl()
                                  * 1000
                          << " ms, msg will be saved and transmitted later in the Tx timeslot to the dest Addr"
                          << endl;

                if (sendMultipurpose) { // if a timeslot is linked to the device
                    if (linkTable->existLink(
                            (UINT_16) tmpMPData->getDstAddr().getInt())) {
                        //send(tmpMPData, mQueueOut);
                        queueModule->insertInQueue(tmpMPData);
                    } else {
                        EV
                                  << "[MAC][TSCH CSMA-CA]: ERROR, no TX link available to the device!"
                                  << endl;
                        delete tmpMPData;
                        numUpperPktLost++;
                    }
                } else {   // if a timeslot is linked to the device
                    if (linkTable->existLink(
                            (UINT_16) tmpData->getDstAddr().getInt())) {
 ...
 *
 * param[in]     "param1 Input parameter (i.e. read by this function). */
void Ieee802154eMac::MCPS_DATA_confirm(UINT_8 msduHandle, UINT_64 timestamp,
        bool rangingReceived, UINT_32 rangingCounterStart,
        UINT_32 rangingCounterStop, UINT_32 rangingTrackingInterval,
        UINT_32 rangingOffset, UINT_8 rangingFOM, MACenum status,
        UINT_8 numBackoffs, UINT_8 dsn, cPacket* ackPayload) {
    cMessage *appframe;
    if (ackPayload == NULL)
...
//
//        EV
//                  << "[MAC]: sending MCPS-DATA.confirm frame to upper layer (disabled), frame retries="
...
//        //control_info->setAckPayload     //
//
//        appframe->setControlInfo(cinfo);
//        EV
//                  << "[MAC]: sending MCPS-DATA.confirm frame to upper layer, the frame retries="
//                  << (int) numBackoffs << " with status=" << statusTxt << endl;
 ...
 *
 * param[in]     "param1 Input parameter (i.e. read by this function). */
void Ieee802154eMac::MCPS_DATA_indication(Ieee802154eAddrMode srcAddrMode,
        UINT_16 srcPANId, IE3ADDR srcAddr, Ieee802154eAddrMode dstAddrMode,
        UINT_16 dstPANId, IE3ADDR dstAddr, UINT_8 msduLength,
        Ieee802154eFrame* msdu, UINT_8 mpduLinkQuality, UINT_8 dsn,
        UINT_32 Timestamp, UINT_8 SecurityLevel, UINT_8 KeyIdMode,
        UINT_64 keySource, UINT_8 keyIndex, UINT_8 uwbPRF,
        Ieee802154eUWBFType uwbPreambleSymbolRepetitions, UINT_8 dataRate,
        RangingControl rangingReceived, UINT_32 rangingCounterStart,
        UINT_32 rangingCounterStop, UINT_32 rangingTrackingInterval,
        UINT_32 rangingOffset, UINT_8 rangingFOM) {
    // [SR] old version
//     if (msdu->getIsGTS())
//     {
//         numRxGTSPkt++;
...
//     {
    ...
    EV << "[MAC]: sending received " << msg->getName()
              << " frame to upper layer" << endl;
...
/**@author: 2014    Stefan Reis     (modified) */
void Ieee802154eMac::csmacaStart(bool firsttime, Ieee802154eFrame* frame,
        bool ackReq) {
    bool backoff;
    ...
    case EXPONENTIAL: {
        ...
        EV << "(startTimer) backoffTimer value=" << backoffTime << " (BE=" << BE
                  << ", 2^BE-1= " << v << ", r=" << r << ")" << endl;
        break;
    }
    case LINEAR: {
    ...
    EV << "[CSMA-CA]: choosing random number of backoff periods: "
              << backoffTime / bPeriod << endl;
    EV << "[CSMA-CA]: backoff time before adjusting: " << backoffTime * 1000
              << " ms" << endl;
    backoffTime = csmacaAdjustTime(backoffTime); // if scheduled backoff ends before CAP, backoffTime should be adjusted
    EV << "[CSMA-CA]: backoff time after adjusting: " << backoffTime * 1000
              << " ms" << endl;
    backoff = true;

    if (beaconEnabled) {
        if (firsttime) {
            backoffTime = csmacaLocateBoundary(toParent(tmpCsmaca),
                    backoffTime);
            EV << "[CSMA-CA]: backoff time after locating boundary: "
                      << backoffTime * 1000 << " ms" << endl;
        }
...
void Ieee802154eMac::csmacaReset(bool bcnEnabled) { // CSMA-CA: Slotted? - Std 802.15.4e-2012 (Figure 11 - CSMA-CA Algorithm) page 13
...
bool Ieee802154eMac::csmacaCanProceed(simtime_t wtime, bool afterCCA) {
    ...
    if (!toParent(tmpCsmaca))       // as a coordinator
        ...
        if (mpib.macBeaconOrder != 15)      // beacon enabled as a coordinator
                {
            t_fCAP = mpib.macBeaconTxTime
                    + (txSfSpec.finalCap + 1) * txSfSlotDuration
                            / phy_symbolrate;
            if (now >= t_fCAP)      // Check 1
                ...
                EV
                          << "[CSMA-CA]: cannot proceed because it's now NOT in CAP (outgoing), resume at the beginning of next CAP"
                          << endl;
                return false;
            } else                    // Check 2
            {
                // calculate num of Backoff periods in CAP except for txBcnDuration
                t_CAP = (txSfSpec.finalCap + 1) * txSfSlotDuration
                        / aUnitBackoffPeriod - txBcnDuration;
                EV
                          << "[CSMA-CA]: total number of backoff periods in CAP (except for txBcnDuration): "
                          << t_CAP << endl;

                // calculate num of Backoff periods from the first backoff in CAP to now+wtime
                tmpf = now + wtime;
                tmpf -= mpib.macBeaconTxTime;
                EV << "[CSMA-CA]: wtime = " << wtime
                          << "; mpib.macBeaconTxTime = " << mpib.macBeaconTxTime
                          << endl;
                tmpf = tmpf / bPeriod;
                EV << "[CSMA-CA]: tmpf = " << tmpf << "; bPeriod  = " << bPeriod
                          << endl;
                t_bPeriods = (UINT_16) (SIMTIME_DBL(tmpf) - txBcnDuration);
                EV << "[CSMA-CA]: t_bPeriods = " << t_bPeriods
                          << "; txBcnDuration = " << (int) txBcnDuration
                          << endl;
    ...
    } else        // as a device
    {
        if (rxBO != 15)     // beacon enabled as a device
            ...
            t_fCAP = schedBcnRxTime
                    + (rxSfSpec.finalCap + 1) * rxSfSlotDuration
                            / phy_symbolrate;
            if (now >= t_fCAP)      // Check 1
                    {
                csmacaWaitNextBeacon = true;
                bPeriodsLeft = 0;   // rechoose random backoff
                EV
                          << "[CSMA-CA]: cannot proceed because it's now NOT in CAP (incoming), resume at the beginning of next CAP"
                          << endl;
                return false;
            } else {
                //
                ...
                t_CAP = (rxSfSpec.finalCap + 1) * rxSfSlotDuration
                        / aUnitBackoffPeriod - rxBcnDuration;
                EV
                          << "[CSMA-CA]: total number of backoff periods in CAP (except for rxBcnDuration): "
                          << t_CAP << endl;
                // calculate num of Backoff periods from the first backoff in CAP to now+wtime
                tmpf = now + wtime;
                tmpf -= schedBcnRxTime;
                ...
                EV << "[CSMA-CA]: t_bPeriods = " << t_bPeriods
                          << "; rxBcnDuration = " << (int) rxBcnDuration
                          << endl;
    ...
    if (!ok) {
        EV
                  << "[CSMA-CA]: cannot proceed because the chosen num of backoffs cannot finish before the end of current CAP, resume at the beginning of next CAP"
                  << endl;
    ...
    if (!afterCCA) {
        ...
        t_transacTime += csmacaLocateBoundary(toParent(tmpCsmaca),
                t_transacTime) - (t_transacTime); // locate boundary for second CCA
        t_transacTime += t_CCATime;             //second CCA time
    }
    t_transacTime += csmacaLocateBoundary(toParent(tmpCsmaca), t_transacTime)
            - (t_transacTime);        // locate boundary for transmission
    ...
    tmpf = now + wtime;
    tmpf += t_transacTime;
    EV << "[CSMA-CA]: the entire transmission is estimated to finish at "
              << tmpf << " s" << endl;
    ...
    if (tmpf > t_fCAP) {
        ok = false;
        EV
                  << "[CSMA-CA]: cannot proceed because the entire transmission cannot finish before the end of current cap"
                  << endl;
    } else {
        ok = true;
        ...
        if (rxBO != 15)                         // beacon enabled
            ...
            if ((rxCAP < now) && (rxCAP + tmpf >= now)) //no more than <aMaxLostBeacons> beacons missed
                ...
                EV << "[MAC]: during receiving a beacon, now is: " << now
                          << ", last beacon is received at " << bcnRxTime
                          << endl;
                while (rxCAP < now)
                    rxCAP += rxBI;
            }
    ...
    if (csmacaWaitNextBeacon)
        if (tmpCsmaca && (!backoffTimer->isScheduled())) {
            ...
            if (bPeriodsLeft == 0)      // backoff again
                    {
                EV
                          << "[CSMA-CA]: bPeriodsLeft == 0, need to rechoose random number of backoffs"
                          << endl;
                csmacaStart(false);
            } else // resume backoff: this is the second entry to CSMA-CA without calling <csmacaStart()>
            {
                /*wtime = csmacaAdjustTime(0.0);
                 EV << "wtime = " << wtime << endl;*/
                EV
                          << "[CSMA-CA]: bPeriodsLeft > 0, continue with the number of backoffs left from last time"
                          << endl;
...
void Ieee802154eMac::handle_MLME_ASSOCIATE_response(cMessage *msg) {
...
void Ieee802154eMac::handle_MLME_DISASSOCIATE_response(cMessage *msg) {
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_COMM_STATUS_indication(UINT_16 panId,
        Ieee802154eAddrMode srcAddrMode, IE3ADDR srcAddr,
        Ieee802154eAddrMode dstAddrMode, IE3ADDR dstAddr, MACenum status,
        UINT_8 securityLevel, UINT_8 keyIdMode, UINT_64 keySource,
        UINT_8 keyIndex) {
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_GET_confirm(MACenum status, MACPIBenum pibAttribute,
 ...
 *
 * param[in] */
void Ieee802154eMac::handle_MLME_GTS_request(UINT_8 gtsCharacteristics,
        UINT_8 securityLevel, UINT_8 keyIdMode, UINT_64 keySource,
        UINT_8 keyIndex) {
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_ORPHAN_indication(IE3ADDR orphanAddress,
        UINT_8 securityLevel, UINT_8 keyIdMode, UINT_64 keySource,
        UINT_8 keyIndex) {
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_ORPHAN_responce(IE3ADDR orphanAddress,
        UINT_16 shortAddress, bool associatedMember, UINT_8 securityLevel,
 ...
 *
 * param[in] */
void Ieee802154eMac::handle_MLME_RX_ENABLE_request(bool deferPermit,
        UINT_32 rxOnTime, UINT_32 rxOnDuration,
        RangingControl rangingRxControl) {
...
void Ieee802154eMac::handle_MLME_SET_request(MACPIBenum pibAttribute,
        MAC_PIB pibAttributeValue) {
    // used the code from the NS2 implementation [SR]
    PHYenum p_state;
    MACenum t_status;
    ...
    case MAC_ACK_WAIT_DURATION: {
        ...
        if (((ppib.phyCurrentChannel <= 10)
                && (pibAttributeValue.macAckWaitDuration != 120))
                || ((ppib.phyCurrentChannel > 10)
    ...
    case MAC_BEACON_PAYLOAD: {
        ...
        memcpy(mpib.macBeaconPayload, pibAttributeValue.macBeaconPayload,
                mpib.macBeaconPayloadLength);
        break;
    }
    case MAC_BEACON_PAYLOAD_LENGTH: {
    ...
    case MAC_RESPONSE_WAIT_TIME: {
        if (pibAttributeValue.macResponseWaitTime < 2
                || pibAttributeValue.macResponseWaitTime > 64) {
            t_status = mac_INVALID_PARAMETER;
        } else {
    ...
    case MAC_SYNC_SYMBOL_OFFSET: {
        PLME_GET_request(PHY_CURRENT_CHANNEL); //value will be returned in ppib
        if (((ppib.phyCurrentChannel <= 10)
                && (pibAttributeValue.macSyncSymbolOffset > 0x400))
                || ((ppib.phyCurrentChannel > 10)
                        && (pibAttributeValue.macSyncSymbolOffset > 0x100))) {
            t_status = mac_INVALID_PARAMETER;
        } else {
    ...
    case MAC_TRANSACTION_PERSISTENCE_TIME: {
        mpib.macTransactionPersistenceTime =
    ...
    case MAC_TX_CONTROL_ACTIVE_DURATION: {
        if (pibAttributeValue.macTxControlActiveDuration > 100000) {
            ...
            mpib.macTxControlActiveDuration =
                    pibAttributeValue.macTxControlActiveDuration;
        }
        break;
    }
    case MAC_TX_CONTROL_PAUSE_DURATION: {
        if (pibAttributeValue.macTxControlPauseDuration != 2000
                || pibAttributeValue.macTxControlPauseDuration != 10000) {
            t_status = mac_INVALID_PARAMETER;
        } else {
            mpib.macTxControlPauseDuration =
                    pibAttributeValue.macTxControlPauseDuration;
        }
        break;
    }
    case MAC_TX_TOTAL_DURATION: {
        ...
        // Security-related MAC PIB attributes - Std 802.15.4-2011 (table 60) page 142
    case MAC_FRAME_COUNTER: {
        if (((mpib.macFrameCounterMode == 0x04)
                && (pibAttributeValue.macFrameCounter > 0xffffffff))
                || ((mpib.macFrameCounterMode == 0x05)
                        && (pibAttributeValue.macFrameCounter > 0xffffffffff))) {
            t_status = mac_INVALID_PARAMETER;
        } else {
    ...
    case MAC_AUTO_REQUEST_SECURITY_LEVEL: {
        mpib.macAutoRequestSecurityLevel =
                pibAttributeValue.macAutoRequestSecurityLevel;
        break;
    }
    case MAC_AUTO_REQUEST_KEY_ID_MODE: {
        if (pibAttributeValue.macAutoRequestKeyIdMode > 0x03) {
            ...
            mpib.macAutoRequestKeyIdMode =
                    pibAttributeValue.macAutoRequestKeyIdMode;
        }
        break;
    }
    case MAC_AUTO_REQUEST_KEY_INDEX: {
        if (pibAttributeValue.macAutoRequestKeyIndex < 0x01) {
            ...
            mpib.macAutoRequestKeyIndex =
                    pibAttributeValue.macAutoRequestKeyIndex;
        }
        break;
    }
    case MAC_PAN_COORD_EXTENDED_ADDRESS: {
        mpib.macPANCoordExtendedAddress =
                pibAttributeValue.macPANCoordExtendedAddress;
        break;
    }
    case MAC_PAN_COORD_SHORT_ADDRESS: {
        mpib.macPANCoordShortAddress =
                pibAttributeValue.macPANCoordShortAddress;
        break;
    }
        // Security-related MAC PIB attributes - Std 802.15.4e-2012 (table 60) page 188
    case MAC_FRAME_COUNTER_MODE: {
        if (pibAttributeValue.macFrameCounterMode < 0x04
                || pibAttributeValue.macFrameCounterMode > 0x05) {
            t_status = mac_INVALID_PARAMETER;
        } else {
...
void Ieee802154eMac::handle_MLME_SYNC_request(UINT_8 channelNumber,
        UINT_8 channelPage, bool trackBeacon) {
...
void Ieee802154eMac::MLME_SYNC_LOSS_indication(MACenum lossReason,
        UINT_16 panId, UINT_8 channelNumber, UINT_8 channelPage,
        UINT_8 securityLevel, UINT_8 keyIdMode, UINT_64 keySource,
        UINT_8 keyIndex) {
...
void Ieee802154eMac::handle_MLME_POLL_request(Ieee802154eAddrMode coordAddrMode,
        UINT_16 coordPANId, IE3ADDR coordAddress, UINT_8 securityLevel,
        UINT_8 keyIdMode, UINT_64 keySource, UINT_8 keyIndex) {
...
void Ieee802154eMac::handle_MLME_DPS_request(UINT_8 txDPSIndex,
        UINT_8 rxDPSIndex, UINT_32 dpsIndexDuration) {
...
void Ieee802154eMac::MLME_SOUNDING_confirm(std::vector<int> soundingList,
        MACenum status) {
 ...
 *
 * param[in]*/
void Ieee802154eMac::MLME_BEACON_REQUEST_indication(
        Ieee802154eBeaconType beaconType, UINT_8 srcAddrMode, IE3ADDR srcAddr,
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_SET_SLOTFRAME_confirm(UINT_8 slotframeHandle,
        MACenum status) {
 ...
 *
 * param[in] */
void Ieee802154eMac::handle_MLME_SET_LINK_request(
        Ieee802154eOperation operation, UINT_16 linkHandle,
        UINT_8 slotframeHandle, UINT_16 timeslot, UINT_16 channelOffset,
        MACTSCHLinkOptions linkOptions, MACTSCHLinkType linkType,
        UINT_16 nodeAddr) {
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_SET_LINK_confirm(MACenum status, UINT_16 linkHandle,
        UINT_8 slotframeHandle) {
 ...
 *
 * param[in] */
void Ieee802154eMac::handle_MLME_TSCH_MODE_request(
        Ieee802154eTSCHMode TSCHMode) {
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_TSCH_MODE_confirm(Ieee802154eTSCHMode TSCHMode,
        MACenum status) {
 ...
 *
 * param[in] */
void Ieee802154eMac::handle_MLME_KEEP_ALIVE_request(UINT_16 dstAddr,
        UINT_16 keepAlivePeriod) {
...
void Ieee802154eMac::startBackoffTimer(simtime_t wtime) {
    EV << "[CSMA-CA]: #" << (int) tmpCsmaca->getSeqNmbr()
              << ": starting backoff for " << wtime << " s" << endl;
...
void Ieee802154eMac::startBcnTxTimer(bool txFirstBcn, simtime_t startTime) {
    ...
    {
        if (mpib.macTSCHenabled) {   // Std 802.15.4e-2012 TSCH mode
            txNow_bcnTxTimer = true;
            beaconWaitingTx = true;
            handleBcnTxTimer();
            return;
        } else {   // Std 802.15.4-2006/2011
    ...
    if (!txNow_bcnTxTimer) {
        if (bcnTxTimer->isScheduled())
            cancelEvent(bcnTxTimer);
        scheduleAt(now + Ieee802154ePhy::aTurnaroundTime / phy_symbolrate,
                bcnTxTimer);
    } else if (mpib.macBeaconOrder != 15 && !mpib.macTSCHenabled) // Std 802.15.4-2006/2011
            {
        wtime = (aBaseSuperframeDuration * (1 << mpib.macBeaconOrder)
                - Ieee802154ePhy::aTurnaroundTime) / phy_symbolrate;
        if (bcnTxTimer->isScheduled())
            cancelEvent(bcnTxTimer);
        scheduleAt(now + wtime, bcnTxTimer);
    }
...
void Ieee802154eMac::startAckTimeoutTimer() {
    ...
    scheduleAt(simTime() + mpib.macAckWaitDuration / phy_symbolrate,
            ackTimeoutTimer);
}
...
void Ieee802154eMac::handleBackoffTimer() {
    EV
              << "[CSMA-CA]: before sending CCA primitive, tell PHY to turn on the receiver"
              << endl;
    taskP.taskStatus(TP_RX_ON_CSMACA) = true;
    PLME_SET_TRX_STATE_request(phy_RX_ON);
}
...
// data or cmd is sent to Phy layer here
{
    ...
    if (txBcnCmd == txCsmaca) {
        ...
        txPkt = txData;
        PD_DATA_request(txData->getByteLength(),
                check_and_cast<Ieee802154eFrame *>(txData->dup()));
    }
}
...
// other cmd pkts requiring no ACK are processed in <handleCommand()>
{
    ...
    if (rxCmd)      // MAC command, all commands requiring ACK are handled here
    {
        ...
        ASSERT(
                taskP.taskStatus(TP_MCPS_DATA_REQUEST)
                        && (taskP.taskStep(TP_MCPS_DATA_REQUEST) == 4));
        FSM_MCPS_DATA_request();
    }
}
...
void Ieee802154eMac::taskSuccess(char type, bool csmacaRes) {
    ...
    if (type == 'b')    //beacon
        ...
        if (fmod(tmpf, aUnitBackoffPeriod) > 0.0)
            txBcnDuration++;
        EV << "[MAC]: calculating txBcnDuration = " << (int) txBcnDuration
                  << endl;

        t_CAP = (txSfSpec.finalCap + 1) * txSfSlotDuration / aUnitBackoffPeriod
                - txBcnDuration;
        ...
        if (t_CAP == 0) // it's possible that there is no time left in current CAP after txing beacon
            ...
            PLME_SET_TRX_STATE_request(phy_TRX_OFF);
            EV
                      << "[MAC]: no time left in current CAP after txing beacon, trying to turn off radio"
                      << endl;
        } else {
            PLME_SET_TRX_STATE_request(phy_RX_ON);
    ...
    /*else if (task == 'c') //command
     ...
     txBcnCmdUpper = NULL;
            ...
            {
        ASSERT(txData);
        EV << "[MAC]: taskSuccess for sending : " << txData->getName() << ":#"
                  << (int) txData->getSeqNmbr() << endl;
        //Packet *p = txData;

        MCPS_DATA_confirm(0, txData->getArrivalTime().getScale(), false, 0, 0,
                0, 0, 0, mac_SUCCESS, 0, 0, txData);
        ...
        csmacaRes = false;
        ASSERT(txGTS);
        EV << "[GTS]: taskSuccess for sending : " << txGTS->getName() << ":#"
                  << (int) txGTS->getSeqNmbr() << endl;
        ...
        MCPS_DATA_confirm(0, txGTS->getArrivalTime().getScale(), false, 0, 0, 0,
                0, 0, mac_SUCCESS, 0, 0, txGTS);
...
void Ieee802154eMac::taskFailed(char type, MACenum status, bool csmacaRes) {
    ...
    || (type == 'a')    //ack.
        ...
        MCPS_DATA_confirm(txData->getSeqNmbr(), 0, false, 0, 0, 0, 0, 0, status,
                0, 0, NULL);
        ...
        if (!taskP.taskStatus(TP_MCPS_DATA_REQUEST))
            ...
            {
        ASSERT(txGTS);
        EV << "[MAC]: taskFailed for sending : " << txGTS->getName() << ":#"
                  << (int) txGTS->getSeqNmbr() << endl;
        // if PAN coordinator, need to clear isTxPending in corresponding GTS descriptor
        if (isPANCoor) {
            ASSERT(gtsList[indexCurrGts].isTxPending);
    ...
    if (txOption == DIRECT_TRANS) {
        ...
        case 3:
            ...
            } else                // time out when waiting for ack.
            {
                ...
                if (numDataRetry <= mpib.macMaxFrameRetries) {
                    EV << "[MAC]: starting " << numDataRetry << "the retry for "
                              << txData->getName() << ":#"
                              << (int) txData->getSeqNmbr() << endl;
        ...
        case 2: // data successfully transmitted
            if (txGTS->getFrmCtrl().ackReq) //ack. required
            {
                ...
                //enable the receiver
                EV
                          << "[GTS]: data successfully transmitted, turn on radio and wait for ACK"
                          << endl;
                PLME_SET_TRX_STATE_request(phy_RX_ON);
                startAckTimeoutTimer();
                waitGTSAck = true;
            ...
            {
                EV
                          << "[GTS]: data successfully transmitted, no ACK required, turn off radio now"
                          << endl;
                PLME_SET_TRX_STATE_request(phy_FORCE_TRX_OFF);
                EV
                          << "[GTS]: need to delay IFS before next GTS transmission can proceed"
                          << endl;
        ...
        case 3:
            if (pStatus == phy_SUCCESS) //ack. received
                    {
                waitGTSAck = false;
                EV << "[GTS]: ACK for " << txGTS->getName() << ":#"
                          << (int) txGTS->getSeqNmbr()
            ...
            {
                EV << "[MAC]: ACK timeout for " << txGTS->getName() << ":#"
                          << (int) txGTS->getSeqNmbr() << endl;
                numGTSRetry++;
                if (numGTSRetry <= mpib.macMaxFrameRetries) {
                    // retry in next GTS
                    EV
                              << "[GTS]: retry in this GTS of next superframe, turn off radio now"
                              << endl;
                    taskP.taskStep(task) = 1;   // go back to step 1
        ...
        case 4:
            taskP.taskStatus(task) = false; // task ends successfully
            EV
                      << "[GTS]: GTS transmission completes successfully, prepared for next GTS request"
                      << endl;
            taskSuccess('g');
            break;
...
void Ieee802154eMac::dispatch(PHYenum pStatus, const char *frFunc,
        PHYenum req_state, MACenum mStatus) {
    FrameCtrl frmCtrl;
    bool isSIFS = false;
    //int i;
    ...
    if (strcmp(frFunc, "csmacaCallBack") == 0) {
        if (txCsmaca == txBcnCmdUpper) {
            ...
            ASSERT(
                    taskP.taskStatus(TP_MCPS_DATA_REQUEST)
                            && (strcmp(taskP.taskFrFunc(TP_MCPS_DATA_REQUEST),
                                    frFunc) == 0));
...
//                else
    ...
    /***************************************/
        ...
        if (txPkt == txData) {
            if ((taskP.taskStatus(TP_MCPS_DATA_REQUEST))
                    && (strcmp(taskP.taskFrFunc(TP_MCPS_DATA_REQUEST), frFunc)
                            == 0))
                FSM_MCPS_DATA_request(pStatus); // mStatus ignored
...
int Ieee802154eMac::calFrmByteLength(Ieee802154eFrame* frame) //
        {
    //EV << "[MAC]: calculating size of " << frame->getName() << endl;

    //Ieee802154eFrame* tmpFrame = check_and_cast<Ieee802154eFrame *>(frame);

    Ieee802154eFrameType frmType = Ieee802154eFrameType(
            frame->getFrmCtrl().frameType);
    int byteLength = 0; // MHR + MAC payload + MFR
    int MHRLength = calMHRByteLength(frame->getFrmCtrl().srcAddrMode,
            frame->getFrmCtrl().dstAddrMode);
    ...
    if (frmType == Ieee802154e_BEACON)  // Std 802.15.4-2006 (figure 10) page 21
        ...
        if (frame->getFrmCtrl().frameVersion == Ieee802154_compatible) {
            ...
            Ieee802154EnhancedBeaconFrame* enhBeaconFrm = check_and_cast<
                    Ieee802154EnhancedBeaconFrame *>(frame);
            if (enhBeaconFrm->getFrmCtrl().presIElist) {
                MHRLength += (enhBeaconFrm->getIeHeaderArraySize() > 0) ? 1 : 0;
                for (unsigned int i = 0;
                        i < enhBeaconFrm->getIeHeaderArraySize(); i++) {
                    MHRLength = MHRLength
                            + enhBeaconFrm->getIeHeader(i).ieLenght;
                }
                MHRLength +=
                        (enhBeaconFrm->getIePayloadArraySize() > 0) ? 1 : 0;
                for (unsigned int i = 0;
                        i < enhBeaconFrm->getIePayloadArraySize(); i++) {
                    MHRLength = MHRLength
                            + enhBeaconFrm->getIePayload(i).ieLenght;
                }
            }
            // beacon payload
            byteLength = MHRLength + (frame->getFrmCtrl().seqNbSup ? 0 : 1) + 2; //SequencNumber + FCS (2 byte)
        } else {
            Ieee802154eBeaconFrame* beaconFrm = check_and_cast<
                    Ieee802154eBeaconFrame *>(frame);
            ...
            byteLength = MHRLength + 2
                    + (beaconFrm->getGtsField().gtsDescrCnt == 0 ? 1 : 2)
                    + beaconFrm->getGtsField().gtsDescrCnt * 3 + 1
                    + beaconFrm->getPendingAdrrFld().numShortAddr * 2
                    + beaconFrm->getPendingAdrrFld().numExtendedAddr * 8 + 2;
        }
        ...
        Ieee802154eMPFrame* tmpMPFrame = check_and_cast<Ieee802154eMPFrame *>(
                frame);
        //
        byteLength = MHRLength
                + (tmpMPFrame->getMpFrmCtrl().lngFrameCtrl ? 1 : 0)
                + (tmpMPFrame->getFrmCtrl().seqNbSup ? 0 : 1)
                + (!tmpMPFrame->getMpFrmCtrl().presPANID ? -4 : 0) + 2;
        ...
        if (frame->getFrmCtrl().frameVersion == Ieee802154_compatible) {
            ...
            Ieee802154eEnhancedAckFrame* enhAckFrm = check_and_cast<
                    Ieee802154eEnhancedAckFrame *>(frame);
            if (enhAckFrm->getFrmCtrl().presIElist) {
                MHRLength += (enhAckFrm->getIeHeaderArraySize() > 0) ? 1 : 0;
                for (unsigned int i = 0; i < enhAckFrm->getIeHeaderArraySize();
                        i++) {
                    MHRLength = MHRLength + enhAckFrm->getIeHeader(i).ieLenght;
                }
                MHRLength += (enhAckFrm->getIePayloadArraySize() > 0) ? 1 : 0;
                for (unsigned int i = 0; i < enhAckFrm->getIePayloadArraySize();
                        i++) {
            ...
            {
        Ieee802154eCmdFrame* cmdFrm = check_and_cast<Ieee802154eCmdFrame *>(
                frame);
        ...
        default:
            error(
                    "[MAC]: cannot calculate the size of a MAC command frame with unknown type!");
            break;
        }
    } else
        error(
                "[MAC]: cannot calculate the size of a MAC frame with unknown type!");
...
bool Ieee802154eMac::getPANIDComp(FrameCtrl frmCtrl, UINT_16 srcPanId,
        UINT_16 dstPanId) {
    if (frmCtrl.srcAddrMode == defFrmCtrl_AddrModeNone
            && frmCtrl.dstAddrMode == defFrmCtrl_AddrModeNone
            && frmCtrl.frameVersion == Ieee802154_compatible && srcPanId == 0
            && dstPanId != 0) {
        return true;
    } else if (frmCtrl.srcAddrMode != defFrmCtrl_AddrModeNone
            && frmCtrl.dstAddrMode == defFrmCtrl_AddrModeNone
            && frmCtrl.frameVersion == Ieee802154_compatible && srcPanId == 0
            && dstPanId == 0) {
        return true;
            ...
            && frmCtrl.dstAddrMode != defFrmCtrl_AddrModeNone
            && frmCtrl.frameVersion == Ieee802154_compatible && srcPanId == 0
            && dstPanId == 0 && frmCtrl.frameType != Ieee802154e_MULTI) {
        return true;
    } else if (frmCtrl.srcAddrMode != defFrmCtrl_AddrModeNone
            && frmCtrl.dstAddrMode != defFrmCtrl_AddrModeNone
            && (frmCtrl.frameVersion == Ieee802154_2003_compatible
                    || frmCtrl.frameVersion == Ieee802154_2006_introduced)
            && srcPanId == 0 && dstPanId != 0) {
        return true;
    } else if (frmCtrl.srcAddrMode != defFrmCtrl_AddrModeNone
            && frmCtrl.dstAddrMode != defFrmCtrl_AddrModeNone
            && frmCtrl.frameVersion == Ieee802154_compatible && srcPanId == 0
...
bool Ieee802154eMac::toParent(Ieee802154eFrame* frame) {
    ...
    if (((tmpFrame->getFrmCtrl().dstAddrMode == defFrmCtrl_AddrMode16)
            && (tmpFrame->getDstAddr().getInt() == mpib.macCoordShortAddress))
            || ((tmpFrame->getFrmCtrl().dstAddrMode == defFrmCtrl_AddrMode64)
                    && (tmpFrame->getDstAddr() == mpib.macCoordExtendedAddress))
            || (tmpFrame->getDstAddr() == (IE3ADDR) 0xffff)) {
        return true;
    } else {
        return false;
    }
...
void Ieee802154eMac::calcTimeCorr(Ieee802154eFrame* frame) {
    double correction = (startTimeslot
            + (timeslotTable->getTemplate(useTimeslotID)->getRxOffsetDbl()
                    + (timeslotTable->getTemplate(useTimeslotID)->getRxWaitDbl()
                            / 2)) - getArrivalTime(frame)).dbl();
    ;
...
//	int test = 0;
    ...
    if (!isPANCoor) // made only the calculation if the
        timeCorrection =
                (timeCorrection == 0.0) ?
                        correction : ((timeCorrection + correction) / 2);
    else
        timeCorrection = 0.0;
}
...
//    uint16_t t_SO;
    ...
    ASSERT(isPANCoor);
    tmpf = mpib.macBeaconTxTime
            + gtsList[index_gtsTimer].startSlot * txSfSlotDuration
                    / phy_symbolrate;
    w_time = tmpf - simTime();
    ...
    EV << "[GTS]: schedule for starting of GTS index: " << index_gtsTimer
              << ", slot:#" << (int) gtsList[index_gtsTimer].startSlot
              << " with " << w_time << " s" << endl;
...
void Ieee802154eMac::handleGtsTimer() {
    ...
    if (isPANCoor) {
        ...
        EV << "[GTS]: GTS with index = " << (int) indexCurrGts
                  << " in my GTS list starts now!" << endl;
        EV << "allocated for MAC address = "
                  << (int) gtsList[indexCurrGts].devShortAddr
                  << ", startSlot = " << (int) gtsList[indexCurrGts].startSlot
                  << ", length = " << (int) gtsList[indexCurrGts].length
                  << ", isRecvGTS = " << gtsList[indexCurrGts].isRecvGTS
                  << ", isTxPending = " << gtsList[indexCurrGts].isTxPending
                  << endl;

        // is transmit GTS relative to device , turn on radio receiver
        if (!gtsList[indexCurrGts].isRecvGTS) {
            EV
                      << "[GTS]: tell PHY to turn on radio receiver and prepare for receiving pkts from device"
                      << endl;
            PLME_SET_TRX_STATE_request(phy_RX_ON);
        }
        // my time to transmit pkts to certain device
            ...
            if (txGTS && gtsList[indexCurrGts].isTxPending) {
                ASSERT(
                        taskP.taskStatus(TP_MCPS_DATA_REQUEST)
                                && taskP.taskStep(TP_MCPS_DATA_REQUEST) == 1);
                ...
                EV
                          << "[GTS]: a data pending for this GTS found in the buffer, starting GTS transmission now"
                          << endl;
            ...
            // turn off radio
                ...
                EV
                          << "[GTS]: no data pending for current transmit GTS, turn off radio now"
                          << endl;
                PLME_SET_TRX_STATE_request(phy_TRX_OFF);
            }
        }
    ...
    // for devices
        ...
        if (index_gtsTimer != 99) {
            ...
            if (isRecvGTS) {
                EV
                          << "[GTS]: tell PHY to turn on radio receiver and prepare for receiving pkts from PAN coordinator"
                          << endl;
                PLME_SET_TRX_STATE_request(phy_RX_ON);
            }
            // my GTS to transmit pkts to PAN coordinator
                ...
                if (txGTS) {
                    ASSERT(
                            taskP.taskStatus(TP_MCPS_DATA_REQUEST)
                                    && taskP.taskStep(TP_MCPS_DATA_REQUEST)
                                            == 1);
                    // hand over to FSM, which will go to next step
                    // no need to call gtsCanProceed() at this time, timing is already guaranteed when applying for GTS
                    EV
                              << "[GTS]: a data is pending for this GTS in the buffer, starting GTS transmission now"
                              << endl;
                    FSM_MCPS_DATA_request();    // state parameters are ignored
                } else {
                    EV
                              << "[GTS]: no data pending for GTS transmission, turn off radio now"
                              << endl;
                    // to avoid several consecutive PLME_SET_TRX_STATE_request are called at the same time, which may lead to error operation,
                    // use phy_FORCE_TRX_OFF to turn off radio, because PHY will not send back a confirm from it
                    PLME_SET_TRX_STATE_request(phy_FORCE_TRX_OFF);
 ...
 */
uint16_t Ieee802154eMac::gts_request_cmd(UINT_16 macShortAddr, uint16_t length,
        bool isReceive) {
    Enter_Method_Silent
    ();
    uint16_t startSlot;
    ...
    ASSERT(isPANCoor);
    EV
              << "[GTS]: the PAN coordinator is processing GTS request from MAC address "
              << macShortAddr << endl;
    // check whether this device is device list
    if (deviceList.find(macShortAddr) == deviceList.end()) {
        EV << "[GTS]: the address " << macShortAddr
                  << " not found in the device list, no GTS allocated" << endl;
        return 0;
    } else if (gtsCount >= 7) {
        EV
                  << "[GTS]: max number of GTSs reached, no GTS available for the MAC address "
                  << macShortAddr << endl;
        return 0;
    }
    ...
    if (t_cap < aMinCAPLength) {
        EV
                  << "[GTS]: violation of the min CAP length, no GTS allocated for the MAC address "
                  << macShortAddr << endl;
        return 0;
    }
    ...
    EV << "[GTS]: add a new GTS descriptor with index = " << (int) gtsCount
              << " for MAC address = " << macShortAddr << ", startSlot = "
              << (int) startSlot << ", length = " << (int) length
...
void Ieee802154eMac::handleFinalCapTimer() {
    ...
    EV
              << "[GTS]: it's now the end of CAP, turn off radio and wait for my GTS coming"
              << endl;
    PLME_SET_TRX_STATE_request(phy_TRX_OFF);
}

UINT_16 Ieee802154eMac::associate_request_cmd(IE3ADDR extAddr,
        const DevCapability& capaInfo) {
    Enter_Method_Silent
    ();
    ...
    EV << "[MAC]: associate request received from " << capaInfo.hostName
              << " with MAC address: " << extAddr << endl;
    return shortAddr;
}
...
bool Ieee802154eMac::gtsCanProceed() {
    simtime_t t_left;
    EV
              << "[GTS]: checking if the data transaction can be finished before the end of current GTS ... ";
...
macLinkTableEntry* Ieee802154eMac::getLinkEntry(UINT_64 asnCounter) {
    if (mpib.macTSCHenabled) {
        ...
        for (int i = 0; i < slotframeTable->getNumSlotframes(); i++) {
            ...
            linkTable->getLinksByTimeslotSlotframe(
                    (asnCounter) % tmpSlotframe->getSlotframeSize(), i,
                    &tmpLinkVector);
            if (tmpLink != NULL) {
        ...
        if (tmpLinkVector.size() == 0) {
            return NULL;
        } else { // filter the links - Std 802.15.4e-2012 (5.1.1.5.4 Multiple slotframes) page 18
            ...
            for (std::list<macLinkTableEntry*>::iterator i =
                    tmpLinkVector.begin(); i != tmpLinkVector.end(); ++i) {
                if (tmpEntry == NULL) {
                    if (queueModule->requestSpcPacket(
                            (IE3ADDR) (*i)->getNodeAddress()) != NULL)
                        return (*i);
                    else
                        tmpEntry = (*i);
                }

                else if (tmpEntry->getLinkOption() > (*i)->getLinkOption()
                        || (tmpEntry->getLinkOption() == (*i)->getLinkOption()
                                && tmpEntry->getSlotframeId()
                                        > (*i)->getSlotframeId()))

                        {

                    if (queueModule->requestSpcPacket(
                            (IE3ADDR) tmpEntry->getNodeAddress()) != NULL)
                        return (tmpEntry);
                    else
                        tmpEntry = (*i);
...
double Ieee802154eMac::getNextActiveTimeslot() {
    if (mpib.macTSCHenabled) {
        ...
        if (maxLoop == 0) { // if no active link is found, wake up next timeslot and search again
            asn = mpib.macASN + 1;
            count = 1;
        }
...
bool Ieee802154eMac::handleIEfield(Ieee802154eFrame* frame) {
    if (mpib.macTSCHcapable) {
        ...
        if (tmpFrame->getFrmCtrl().presIElist) {
            for (unsigned int i = 0; i < tmpFrame->getIeHeaderArraySize();
                    i++) {
                //
                ...
                if (tmpHeader.elementID == IE_HD_LE_CSL) {
                    ...
                    break;
                } else if (tmpHeader.elementID
                        == IE_HD_ACK_NACK_TIME_CORRECTION) {
                    double tmpTime = 0.0;
                    UINT_16 tmpTSI = 0x0000;
                    if (isPANCoor
                            || neighborTable->isNeighborTimeSource(
                                    getShortAddress(frame->getSrcAddr())))
                        break;
                    else {
                        if (tmpHeader.ieACKNACKTimeCorrc != NULL
                                && tmpHeader.ieACKNACKTimeCorrc->timeSyncInfo
                                        > 0) {
                            tmpTSI = tmpHeader.ieACKNACKTimeCorrc->timeSyncInfo;
                        }
                        if (tmpTSI > 0x8000) { //NACK
                            isNACK = true;
                    ...
                    break;
                        ...
                        == IE_HD_LOW_LATENCY_NETWORK_INFO) {
                    break;
                } else if (tmpHeader.elementID == IE_HD_LIST_TERMINATION_1) { // followed by payload IE's
                    break;
                } else if (tmpHeader.elementID == IE_HD_LIST_TERMINATION_2) { // followed by unformatted payload
                    break;
                }
            }

            for (unsigned int i = 0; i < tmpFrame->getIePayloadArraySize();
                    i++) {
                ...
                if (tmpPayload.groupID == IE_PL_ESDU) {
                    ...
                    for (it = tmpMLME->ieMLMESub.begin();
                            it != tmpMLME->ieMLMESub.end(); ++it) {
                        if ((*it)->ieMLMEtype == false) //short
                                {
                            if ((*it)->ieMLMESubID
                                    == IE_MLME_SHORT_TSCH_SYNCHRONISATION) {
                                ...
                                //
                            } else if ((*it)->ieMLMESubID
                                    == IE_MLME_SHORT_TSCH_SLOTFRAME_LINK) {
                                //
                                //for(int i = 0; i < (*it)->ieTSCHSLotfrmLink->)
                                slotframeLinkIEEntry *tmpSlotLink =
                                        (*it)->ieTSCHSLotfrmLink->getSlotframeLinkIEEntry();
                                for (int i = 0;
                                        i < tmpSlotLink->getNumSlotframes();
                                        i++) {
                                    macSlotframeTableEntry *tmpSlot =
                                            new macSlotframeTableEntry();
                                    tmpSlot->setSlotframeId(
                                            tmpSlotLink->getSlotframeIE(i)->getSlotframeId());
                                    tmpSlot->setSlotframeSize(
                                            tmpSlotLink->getSlotframeIE(i)->getSlotframeSize());
                                    if (slotframeTable->getSlotframeById(
                                            tmpSlotLink->getSlotframeIE(i)->getSlotframeId()) == NULL)
                                    ...
                                    for (int k = 0;
                                            k
                                                    < tmpSlotLink->getSlotframeIE(
                                                            i)->getNumLinks();
                                            k++) {
                                        if (linkTable->getLinkByTimeslotSlotframe(
                                                tmpSlotLink->getSlotframeIE(i)->getLinkIE(
                                                        k)->getTimeslot(),
                                                tmpSlotLink->getSlotframeIE(i)->getSlotframeId()) == NULL) {
                                            macLinkTableEntry *tmpLink =
                                                    new macLinkTableEntry();
                                            tmpLink->setChannelOffset(
                                                    tmpSlotLink->getSlotframeIE(
                                                            i)->getLinkIE(k)->getChannelOffset());
                                            tmpLink->setLinkOption(
                                                    tmpSlotLink->getSlotframeIE(
                                                            i)->getLinkIE(k)->getLinkOption());

                                            tmpLink->setLinkType(
                                                    tmpSlotLink->getSlotframeIE(
                                                            i)->getLinkIE(k)->getLinkTyp());

                                            if (tmpSlotLink->getSlotframeIE(i)->getLinkIE(
                                                    k)->getLinkTyp()
                                                    == (LNK_TP_ADVERTISING
                                                            || LNK_TP_JOIN))
                                                tmpLink->setNodeAddress(0xffff);
                                            else
                                                tmpLink->setNodeAddress(
                                                        getShortAddress(
                                                                tmpFrame->getSrcAddr()));

                                            tmpLink->setSlotframeId(
                                                    tmpSlotLink->getSlotframeIE(
                                                            i)->getSlotframeId());
                                            tmpLink->setTimeslot(
                                                    tmpSlotLink->getSlotframeIE(
                                                            i)->getLinkIE(k)->getTimeslot());
                                            tmpLink->setLinkId(
                                                    linkTable->getNumLinks());
                                            ...
                                            tmpLink->setTimeslot(
                                                    tmpSlotLink->getSlotframeIE(
                                                            i)->getLinkIE(k)->getTimeslot());
                                            tmpLink->setLinkId(
                                                    linkTable->getLinkByTimeslotSlotframe(
                                                            tmpSlotLink->getSlotframeIE(
                                                                    i)->getLinkIE(
                                                                    k)->getTimeslot(),
                                                            tmpSlotLink->getSlotframeIE(
                                                                    i)->getSlotframeId())->getLinkId());
                                            tmpLink->setMacLinkTable(linkTable);
                                    ...
                                    == IE_MLME_SHORT_TSCH_TIMESLOT) {
                                macTimeslotTableEntry* tmpTimeslot =
                                        (*it)->ieTSCHTimeslt;

                                if (timeslotTable->getNumTemplates() == 0) {
                                    tmpTimeslot->setTemplateId(
                                ...
                                } else if (tmpTimeslot->getTemplateId() + 1
                                        > timeslotTable->getNumTemplates()) {
                                    EV
                                ...
                                if ((*it)->ieNestedMLMELength > 1) {
                                    timeslotTable->editTimeslotTemplate(
                                            tmpTimeslot);
                                }
                            } else if ((*it)->ieMLMESubID
                                    == IE_MLME_SHORT_HOPPING_TIMING) {

                            } else if ((*it)->ieMLMESubID
                                    == IE_MLME_SHORT_EB_FILTER) {

                            } else if ((*it)->ieMLMESubID
                                    == IE_MLME_SHORT_MAC_METRICS_1) {

                            } else if ((*it)->ieMLMESubID
                                    == IE_MLME_SHORT_MAC_METRICS_2) {

                            }
                        } else    // long
                        {
                            if ((*it)->ieMLMESubID
                                    == IE_MLME_LONG_CHANNEL_HOPPING_SEQUENCE) {
                                //fixed
                                IE_CHANNEL_HOPPING* tmpChannel =
                                        (*it)->ieChannelHop;
                                macHoppingSequenceListEntry *tmpHopE =
                                        new macHoppingSequenceListEntry;
                                tmpHopE->setChannelPage(
                                        tmpChannel->channelPage);
                                tmpHopE->setCurrentHop(tmpChannel->currentHop);
                                tmpHopE->setExtendedBitmap(
                                        tmpChannel->extBitmap);
                                tmpHopE->setHopDwellTime(0);
                                tmpHopE->setHoppingSequenceId(
                                        tmpChannel->hoppingSeqID);
                                tmpHopE->setHoppingSequenceLength(
                                        tmpChannel->hoppingSeqLength);
                                tmpHopE->setHoppingSequenceList(
                                        tmpChannel->hoppingSeqLst);
                                tmpHopE->setNumberOfChannels(
                                        tmpChannel->numberOfChannels);
                                tmpHopE->setPhyConfiguration(
                                        tmpChannel->phyConfig);

                                if (hoppingSequenceList->getEntryById(
                                        tmpHopE->getHoppingSequenceId()) == NULL)
                                    hoppingSequenceList->addEntry(tmpHopE);
                                else
                                    hoppingSequenceList->editHoppingSequenceListEntry(
                                            tmpHopE);
                                useHoppingSequenceID =
                                        tmpHopE->getHoppingSequenceId();
                            }
                        }
                    }
...
void Ieee802154eMac::handleAsnTimer() { //[SR] some dummy values and a fixed timeslot duration is implemented
    if (mpib.macTSCHenabled && !taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
            && !taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA)) {
        ...
        EV << "[TSCH CCA]-Time: ASN=" << tmpASN << "; timeCorrection="
                  << timeCorrection << "s; next ASN count=" << (int) mpib.macASN
                  << " next active time=" << simTime() + nextTime << "s"
                  << endl;
        ...
        case LNK_TP_NORMAL:
            ...
            tmpMsg = queueModule->requestSpcPacket(
                    (IE3ADDR) activeLinkEntry->getNodeAddress());
            break;
            ...
            if (txData != NULL) {
                ...
                string tmpTxt =
                        (txData->getFrmCtrl().frameType == Ieee802154e_DATA) ?
                                txData->getEncapsulatedPacket()->getName() :
                                frameTypeTxt[txData->getFrmCtrl().frameType];
                EV << "[TSCH CCA]-Transmitter: " << retries
                          << " retries for this " << txData->getName() << "("
                          << tmpTxt << ") frame" << endl;

                if (tschSharedLink
                        && backoffMethod
                                == EXPONENTIAL&& retries <= mpib.macMaxFrameRetries && retries > 0 && activeNeighborEntry != NULL) { // in TSCH mode the device shall use an exponential backoff mechanism - Std 802.15.4e-2012 (5.1.1.4.3 TSCH CSMA-CA algorithm) page 14
                    int td = activeNeighborEntry->getTransDelay(); // transmission delayed

                    if (td < 0) { // calculation from the cmsa802154.cc file
                        double d = pow((double) 2, (int) BE);
                        int v = (int) d - 1;
                        ...
                        EV
                                  << "[TSCH CCA]-Transmitter (shared links): retransmission delayed for "
                                  << r << " for shared links" << endl;
                    }

                    // check if we need to delay the transmission
                    if (td > 0) {
                        EV
                                  << "[TSCH CCA]-Transmitter (shared links): transmission delay for "
                                  << td << " shared links" << endl;
...
//			return;
                        ...
                        EV << "[TSCH CCA]-Receiver:[1] wait the TsRxOffset"
                                  << endl;
                        scheduleAt(
                                simTime()
                                        + timeslotTable->getTemplate(
                                                useTimeslotID)->getRxOffsetDbl(),
                                tsRxOffsetTimer);
                        return;
                    } else
                ...
                if (useCCA) {
                    ...
                    EV
                              << "[TSCH CCA]-Transmitter:[1] use CCA, wait the TsCCAOffsetTime"
                              << endl;
                    scheduleAt(
                            simTime()
                                    + timeslotTable->getTemplate(useTimeslotID)->getCCAOffsetDbl(),
                            tsCCAOffsetTimer);
                    ...
                    EV
                              << "[TSCH CCA]-Transmitter:[100] no CCA, wait the TsTxOffsetTime"
                              << endl;
                    scheduleAt(
                            simTime()
                                    + timeslotTable->getTemplate(useTimeslotID)->getTxOffsetDbl(),
                            tsTxOffsetTimer);
                    // next step in handleTsTxOffset()
                }
            } else {
                EV << "[TSCH CCA]-Transmitter: no pkt to send, go back to sleep"
                          << endl;
                taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA) = false;
                taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) = 0;
        ...
        } else if (linkOption == LNK_OP_RECEIVE
                || linkOption == LNK_OP_RECEIVE_TIMEKEEPING) // Receiving Link
                        {
            taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA) = true;
            taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) = 1;
            // start the macTsRxOffset timer
        ...
        {
            EV << "[TSCH CCA]-Reserved: nothing to do, go back to sleep"
                      << endl;
            resetTRX();
        }
    } else {
...
void Ieee802154eMac::handleTsCCAOffset() {
    if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
        ...
        PHY_PIB tmpPIB;
        std::vector<UINT_16> tmpHoppingList = hoppingSequenceList->getEntryById(
                useHoppingSequenceID)->getHoppingSequenceList();
        int nextChannel = tmpHoppingList[(curASN
                + activeLinkEntry->getChannelOffset()) % tmpHoppingList.size()];
        tmpPIB.phyCurrentChannel = nextChannel;
        EV
                  << "[TSCH CCA]-Transmitter:[2] before sending CCA primitive, tell the PHY layer to change the current Channel ("
                  << nextChannel << ")" << endl;
...
void Ieee802154eMac::handleTsTxOffset() {
    if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
        ...
        EV
                  << "[TSCH CCA]-Transmitter:[101](without CCA) tell the PHY layer to change the current Channel"
                  << endl;
        PHY_PIB tmpPIB;
        std::vector<UINT_16> tmpHoppingList = hoppingSequenceList->getEntryById(
                useHoppingSequenceID)->getHoppingSequenceList();
        tmpPIB.phyCurrentChannel = tmpHoppingList[(curASN
                + activeLinkEntry->getChannelOffset()) % tmpHoppingList.size()];
        PLME_SET_request(PHY_CURRENT_CHANNEL, tmpPIB);
        // next step in handle_PLME_SET_confirm()
    }
...
void Ieee802154eMac::handleTsRxAckDelay() {
    if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
        ...
        EV
                  << "[TSCH CCA]-Transmitter:[8] set the TRX_STATE to RX_ON and wait for the Ack"
                  << endl;
        PLME_SET_TRX_STATE_request(phy_RX_ON);
        // next step in PLME_SET_TRX_STATE_confirm()
    }
...
void Ieee802154eMac::handleTsAckWait() {
    if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA)
        ...
        if (retries > mpib.macMaxFrameRetries) // failed retransmission; max retransmission for this packet
                {
            EV
                      << "[TSCH CCA]-Transmitter:[12] no Ack for this transmission arrived in the TsAckWaitTime, maximum retries arrived, drop this frame"
                      << endl;
            ...
            MCPS_DATA_confirm(txData->getSeqNmbr(), 0, false, 0, 0, 0, 0, 0,
                    mac_NO_ACK, txData->getRetries(), 0, NULL);
            queueModule->deleteMsgQueue(txData->getDstAddr(), false);
            ...
            if (mpib.macMetricsEnabled)
                mpib.macTXFailCount++;
        } else { // failed retransmission: retry the next time
            if (taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 5)
                EV
                          << "[TSCH CCA]-Transmitter:[12] data transmission failed, go back to sleep, try the next timeslot to the dest address"
                          << endl;
            else if (taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 99)
                EV
                          << "[TSCH CCA]-Transmitter:[12] CCA failed, go back to sleep, try the next timeslot to the dest address"
                          << endl;
            else
                EV
                          << "[TSCH CCA]-Transmitter:[12] no Ack for this transmission arrived in the TsAckWaitTime, try the next timeslot to the dest address"
                          << endl;
        }
...
void Ieee802154eMac::handleTsRxOffset() {
    if (taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA)
        ...
        EV
                  << "[TSCH CCA]-Receiver:[2] tell the PHY layer to change the current Channel"
                  << endl;
...
void Ieee802154eMac::handleTsRxWait() {
    if (taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA)
        ...
        EV
                  << "[TSCH CCA]-Receiver:[6] no correct frame arrived in the TsRxWaitTime"
                  << endl;
        resetTRX();
    }
}
...
void Ieee802154eMac::updateNeighbor(Ieee802154eFrame *frame,
        FrameCtrl frmCtrl) {
    if (!neighborTable->isNeighborBySAddr(getShortAddress(frame->getSrcAddr()))) // if not in neighborTable add it
        ...
        if (frame->getFrmCtrl().frameType == Ieee802154e_BEACON) {
            ...
            tmpEntry->setIsTimeSource(
                    linkTable->isTimescource(
                            getShortAddress(frame->getSrcAddr())));
        }
    ...
    {
        macNeighborTableEntry* tmpEntry = neighborTable->getNeighborBySAddr(
                getShortAddress(frame->getSrcAddr()));
        tmpEntry->setExtendedAddress(frame->getSrcAddr());
