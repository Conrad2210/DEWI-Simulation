...
{
    MACAddress(0xffff)/*def_macExtendedAddress*/, def_macAckWaitDuration,
    def_macAssociatedPANCoord,                  def_macAssociationPermit,
    ...
    def_macMaxBE,                               def_macMaxCSMABackoffs,
    def_macMaxFrameRetries,                     def_macMinBE,
    def_macLIFSPeriod,                          def_macSIFSPeriod,
    def_macPANId,                               def_macPromiscuousMode,
    def_macResponseWaitTime,                    def_macRxOnWhenIdle,
    def_macSecurityEnabled,                     def_macEnhAckWaitDuration,
    def_macImplicitBroadcast,                   def_macSimpleAddress,
    def_macShortAddress,                        def_macSuperframeOrder,
    0 /*def_macSyncSymbolOffset*/,              0 /*def_macTimestampSupported*/,
    def_macTransactionPersistenceTime,          0 /*def_macTxControlActiveDuration*/,
    0 /*def_macTxControlPauseDuration*/,        def_macTxTotalDuration,

    def_macFrameCounter,                        def_macAutoRequestSecurityLevel,
    def_macAutoRequestKeyIdMode,                def_macAutoRequestKeyIndex,
    MACAddress::BROADCAST_ADDRESS64/*macPANCoordExtendedAddress*/,
    def_macPANCoordShortAddress,                def_macFrameCounterMode,

    def_macTSCHcapable,                         def_macLLcapable,
    def_macDSMEcapable,                         def_macLEcapable,
    def_macRFIDcapable,                         def_macHoppingCapable,
    def_macAMCACapable,                         def_macMetricsCapable,
    def_macTSCHenabled,                         def_macLLenabled,
    def_macDSMEenabled,                         def_macLEenabled,
    def_macRFIDenabled,                         def_macHoppingEnabled,
    def_macAMCAenabled,                         def_macMetricsEnabled,

    def_macDisconnectTime,                      def_macJoinPriority,
    def_macASN,                                 def_macNoHLBuffers,

    0/*def_macCounterOctets*/,                  def_macRetryCount,
    def_macMultipleRetryCount,                  def_macTXFailCount,
    def_macTXSuccessCount,                      def_macFCSErrorCount,
    def_macSecurityFailure,                     def_macDuplicateFrameCount,
    def_macRXSuccessCount,                      def_macNACKcount,

    def_macUseEnhancedBeacon,                   def_macEBFilteringEnabled,
    0/*def_macEBSN*/,                           def_macEBAutoSA
};
...
{
        "unknown type",             "Ieee802154e_BEACON",
        "Ieee802154e_DATA",         "Ieee802154e_ACK",
        "Ieee802154e_CMD",          "Ieee802154e_LLDN",
        "Ieee802154e_MULTI"
};
...
{
        "phy_BUSY",                 "phy_BUSY_RX",
        "phy_BUSY_TX",              "phy_FORCE_TRX_OFF",
        "phy_IDLE",                 "phy_INVALID_PARAMETER",
        "phy_RX_ON",                "phy_SUCCESS",
        "phy_TRX_OFF",              "phy_TX_ON",
        "phy_UNSUPPORT_ATTRIBUTE",  "phy_READ_ONLY"
};
...
{
        "PHY_INVALID_PARAMETER",            "PHY_CURRENT_CHANNEL",
        "PHY_CHANNELS_SUPPORTED",           "PHY_TRANSMIT_POWER",
        "PHY_TX_POWER_TOLERANCE",           "PHY_TX_POWER",
        "PHY_CCA_MODE",                     "PHY_CURRENT_PAGE",
        "PHY_MAX_FRAME_DURATION",           "PHY_SHR_DURATION",
        ...
        "PHY_UWB_COU_PULSE",                "PHY_UWB_CS_PULSE",
        "PHY_UWB_LCP_WEIGHT_1",             "PHY_UWB_LCP_WEIGHT_2",
        "PHY_UWB_LCP_WEIGHT_3",             "PHY_UWB_LCP_WEIGHT_4",
        "PHY_UWB_LCP_DELAY_2",              "PHY_UWB_LCP_DELAY_3",
        "PHY_UWB_LCP_DELAY_4",              "PHY_RANGING",
        "PHY_RANGING_CRYSTAL_OFFSET",       "PHY_RANGING_DPS",
        "PHY_CURRENT_CODE",                 "PHY_NATIVE_PRF",
        "PHY_UWB_SCAN_BINS_PER_CHANNEL",    "PHY_UWB_INSERTED_PREAMBLE_INTERVAL",
        "PHY_TX_RMARKER_OFFSET",            "PHY_RX_RMARKER_OFFSET",
        "PHY_RFRAME_PROCESSING_TIME",       "PHY_CCA_DURATION"
};
...
void Ieee802154eMac::initialize(int stage)
{
    ...
    if (0 == stage)
    {
        ...
        mpib                    = MPIB;             // copy the default values
...
//        ack4Gts                 = par("ack4Gts");
    ...
    else if (3 == stage)
    {
        ...
        {   // Start PAN Coordinator - Std 802.15.4-2006 (7.5.2.3.1) page 177
            ...
            EV << "Channel Number: " << ppib.phyCurrentChannel << "; bit rate: " << phy_bitrate /1000 <<
                    " kb/s; symbol rate: " << phy_symbolrate/1000 << " ksymbol/s" << endl;
            ...
            else
            {
                EV << "BO = " << (int)mpib.macBeaconOrder << ", BI = " <<
                        ...
                        " s; duty cycle = " << pow(2.0, (mpib.macSuperframeOrder-mpib.macBeaconOrder))*100 << "%" << endl;
                EV << "There are " << (int)aBaseSuperframeDuration * (1 << mpib.macSuperframeOrder) <<
                        " symbols (" <<  (int)aBaseSuperframeDuration * (1 << mpib.macSuperframeOrder)/aUnitBackoffPeriod <<
...
//                HOPP_DESC hoppingDescriptor);
    ...
    case TP_MLME_SYNC_REQUEST:
        ...
        handle_MLME_SYNC_request(primitive->getChannelNumber(),
                primitive->getChannelPage(),
                primitive->getTrackBeacon());
        delete primitive;
        break;

    case TP_MLME_POLL_REQUEST:
        EV <<"[MAC]: this is a MLME_POLL_REQUEST" << endl;
        handle_MLME_POLL_request((Ieee802154eAddrMode) primitive->getCoordAddrMode(),
                primitive->getCoordPANId(),
                (IE3ADDR) primitive->getCoordAddress(),
                primitive->getSecurityLevel(),
                primitive->getKeyIdMode(),
    ...
    case TP_MLME_DPS_REQUEST:
        ...
        handle_MLME_DPS_request(primitive->getTxDPSIndex(),
                primitive->getRxDPSIndex(),
                primitive->getDpsIndexDuration());
        delete primitive;
        break;
...
void Ieee802154eMac::handleUpperMsg(cPacket* msg) // MCPS-SAP
{
        ...
        else        //  packet destined for device
        {
            ...
            if (mpib.macShortAddress != 0xfffe) // device is associated to a PAN coordinator
            {
                ...
                if (mpib.macShortAddress != 0xfffe
                        && deviceList.find(getShortAddress(destAddr)) == deviceList.end()
                        && !neighborTable->isNeighborBySAddr(getShortAddress(destAddr)))
                {
    ...
    else // for the StarRouting Network layer - for the StarNet and Nodes Random Direct example
    {
        ...
        else        //  packet destined for device
        {
            ...
            cModule* module = simulation.getModuleByPath(cInfo->getDestName())->getModuleByPath(".wlan.mac");
    #else
    ...
    MCPS_DATA_request((Ieee802154eAddrMode)srcAddrMode,
            (Ieee802154eAddrMode)(destAddr.getFlagEui64()?defFrmCtrl_AddrMode64:defFrmCtrl_AddrMode16),
            destPanId, destAddr, (UINT_8)msg->getByteLength(), msg, mpib.macDSN++/*msduHandle (FIXME [SR] we have no msduHandle from the NETWORK layer)*/,
            ackTX, gtsTX, indirectTX,
            secuData/*securityLevel*/, 0/*keyIdMode*/, 0/*keySource*/, 0/*keyIndex*/, UWBPRF_PRF_OFF, RANGING_OFF,
...
void Ieee802154eMac::handleLowerMsg(cPacket* msg) // PD_SAP
{
    ...
    if (frameFilter(frame))
    {
        ...
        if (mpib.macTSCHenabled)
        {
            if (taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA) && taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 10)
            {
                taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) = 11;
...
//    }
    ...
    if ((frmType == Ieee802154e_DATA) || (frmType == Ieee802154e_CMD) || (frmType == Ieee802154e_MULTI))
    {
...
void Ieee802154eMac::sendDown(Ieee802154eFrame* frame)
{
    ...
    EV << "[MAC]: sending frame " <<
            (frame->getFrmCtrl().frameType!=Ieee802154e_DATA?(frame->getName()):
...
void Ieee802154eMac::handleBeacon(Ieee802154eFrame* frame)
{
    ...
    #if OMNETPP_VERSION >= 0x403
            cModule* module = simulation.getModuleByPath(panCoorName)->getModuleByPath(".wlan.mac");
    #else
...
void Ieee802154eMac::handleData(Ieee802154eFrame* frame)
{
    ...
    if (mpib.macTSCHenabled && frmCtrl.frameVersion == Ieee802154_compatible) // 802.15.4e Data frame
    {
        ...
        MCPS_DATA_indication((Ieee802154eAddrMode)rxData->getFrmCtrl().srcAddrMode, rxData->getSrcPanId(),
                rxData->getSrcAddr(), (Ieee802154eAddrMode)rxData->getFrmCtrl().dstAddrMode,
                rxData->getDstPanId(), rxData->getDstAddr(), rxData->getByteLength(),
                rxData, 0 /*mpduLinkQuality*/, rxData->getSeqNmbr(),
                (UINT_32)rxData->getTimestamp().raw(), rxData->getAuxSecHd().secLvl,
                rxData->getAuxSecHd().keyIdentMode, rxData->getAuxSecHd().keySource,
                rxData->getAuxSecHd().keyIndex, 0 /*uwbPRF*/,
                (Ieee802154eUWBFType)0 /*uwbPreambleSymbolRepetitions*/, 0 /*dataRate*/,
                (RangingControl)0 /*rangingReceived*/,
                0 /*rangingCounterStart*/, 0 /*rangingCounterStop*/,
                0 /*rangingTrackingInterval*/, 0 /*rangingOffset*/, 0 /*rangingFOM*/);
        ...
        if (frame->getFrmCtrl().frameVersion == Ieee802154_compatible)  // 802.15.4e Enhanced ACK frame
        {
            ...
            if (mpib.macTSCHenabled && taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA) && taskP.taskStep(TP_TS_TX_CCA_TSCHCCA) == 10)
            {
                ...
                else // successful transmission, with ACK
                {
                    ...
                    EV << "[TSCH CCA]-Transmitter:[10] the correct ACK arrives, transmission of the "
                            << txPkt->getName() << "(" << tmpTxt << ") frame was successful, delete frame from queue" << endl;
...
bool Ieee802154eMac::frameFilter(Ieee802154eFrame* frame)
{
    ...
    */
    ...
    if (!mpib.macPromiscuousMode)
    {
        //check packet type
        if ((frmType != Ieee802154e_BEACON)
                &&(frmType != Ieee802154e_DATA)
                &&(frmType != Ieee802154e_ACK)
                &&(frmType != Ieee802154e_CMD)
        ...
        {   // for Std 802.15.4-2016 & Std 802.15.4-2011
...
void Ieee802154eMac::constructBCN()
{
    ...
    if(mpib.macTSCHenabled && mpib.macUseEnhancedBeacon)
    {
        ...
        tmpTSCHSync->tschASN                = mpib.macASN;
...
//[SR] old version
    if (taskP.taskStatus(TP_CCA_CSMACA))
    {
        ...
        else    // busy channel
        {
            ...
            {
                BE++;
                if (BE > mpib.macMaxBE) BE = mpib.macMaxBE;
...
void Ieee802154eMac::handle_PLME_SET_TRX_STATE_confirm(PHYenum status)
{
    EV << "[MAC]: a PLME_SET_TRX_STATE_confirm with " << stateTxt[status]
        << " received from PHY, the requested state is " << stateTxt[trx_state_req] << endl;
    ...
    if (mpib.macTSCHenabled)
    {
        ...
        else if (trx_state_req == phy_TX_ON)
        {
            ...
            else if (taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA) && taskP.taskStep(TP_TS_RX_CCA_TSCHCCA) == 8) // TSCH CCA Receiver
            {
                ...
                {
                    EV << "[TSCH CCA]-Receiver:[9-ERROR]: the transmission duration ("
                            << calDuration(txAck) << " s ) is bigger than the macTsMaxAck ("
                            << timeslotTable->getTemplate(useTimeslotID)->getMaxAckDbl() *1000 << " ms)" << endl;
 ...
 *
 * param[in]     "param1 Input parameter (i.e. read by this function). */
void Ieee802154eMac::MCPS_DATA_request(Ieee802154eAddrMode srcAddrMode, Ieee802154eAddrMode dstAddrMode, UINT_16 dstPANId,
                                        IE3ADDR dstAddr, UINT_8 msduLength, cPacket* msdu, UINT_8 msduHandle,
                                        bool ackTX, bool gtsTX, bool indirectTX, UINT_8 securityLevel, UINT_8 keyIdMode,
                                        UINT_64 keySource, UINT_8 keyIndex, Ieee802154eUWBFType uwbPRF,
                                        RangingControl ranging, UINT_16 uwbPreambleSymbolRepetitions, UINT_8 dataRate,
                                        FrameCrlOptions frameControlOptions, std::vector<Ieee802154eIEHeaderType> headerIElist,
                                        std::vector<Ieee802154eIEPayloadType> payloadIElist, bool sendMultipurpose)
{
...
//        tmpMPData->setIsGTS(gtsTX);
        ...
        EV << "[MAC]: MPDU constructed: "<< tmpMPData->getName() << " (" << tmpMPData->getEncapsulatedPacket()->getName() << "), #"
            << (int)tmpMPData->getSeqNmbr() << ", " << tmpMPData->getByteLength() << " Bytes" << endl;
    }
    else
    {
        ...
        EV << "[MAC]: MPDU constructed: "<< tmpData->getName() << " (" << tmpData->getEncapsulatedPacket()->getName() << "), #"
            << (int)tmpData->getSeqNmbr() << ", " << tmpData->getByteLength() << " Bytes" << endl;
    }
    ...
    switch (TxOption)
    {
        case DIRECT_TRANS:  // send in the CAP or at the Link for TSCH transmission
        {
            ...
            if (mpib.macTSCHenabled)
            {
                ...
                {
                    EV << "[TSCH CCA]: the estimated transmission time (" << duration << " s) is bigger then " <<
                            timeslotTable->getTemplate(useTimeslotID)->getMaxTxDbl() * 1000
                            << " ms, msg will be discarded" << endl;
                ...
                {
                    EV << "[TSCH CCA]: the estimated transmission time (" << duration << " s) is smaller or equal as the " <<
                            timeslotTable->getTemplate(useTimeslotID)->getMaxTxDbl() * 1000
                            << " ms, msg will be saved and transmitted later in the Tx timeslot to the dest Addr" << endl;
 ...
 *
 * param[in]     "param1 Input parameter (i.e. read by this function). */
void Ieee802154eMac::MCPS_DATA_confirm(UINT_8 msduHandle, UINT_64 timestamp, bool rangingReceived, UINT_32 rangingCounterStart,
                                        UINT_32 rangingCounterStop, UINT_32 rangingTrackingInterval, UINT_32 rangingOffset,
                                        UINT_8 rangingFOM, MACenum status, UINT_8 numBackoffs, UINT_8 dsn,
                                        cPacket* ackPayload)
{
    cMessage *appframe;
    if (ackPayload == NULL)
 ...
 *
 * param[in]     "param1 Input parameter (i.e. read by this function). */
void Ieee802154eMac::MCPS_DATA_indication(Ieee802154eAddrMode srcAddrMode, UINT_16 srcPANId, IE3ADDR srcAddr,
                                        Ieee802154eAddrMode dstAddrMode, UINT_16 dstPANId, IE3ADDR dstAddr,
                                        UINT_8 msduLength, Ieee802154eFrame* msdu, UINT_8 mpduLinkQuality, UINT_8 dsn,
                                        UINT_32 Timestamp, UINT_8 SecurityLevel, UINT_8 KeyIdMode, UINT_64 keySource,
                                        UINT_8 keyIndex, UINT_8 uwbPRF, Ieee802154eUWBFType uwbPreambleSymbolRepetitions,
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_BEACON_NOTIFY_indication(UINT_8 bsn, PAN_ELE panDescriptor, PendingAddrFields pendAddrSpec,
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_COMM_STATUS_indication(UINT_16 panId, Ieee802154eAddrMode srcAddrMode, IE3ADDR srcAddr,
        Ieee802154eAddrMode dstAddrMode, IE3ADDR dstAddr, MACenum status, UINT_8 securityLevel, UINT_8 keyIdMode,
 ...
 *
 * param[in] */
void Ieee802154eMac::MLME_ORPHAN_responce(IE3ADDR orphanAddress, UINT_16 shortAddress, bool associatedMember, UINT_8 securityLevel,
        UINT_8 keyIdMode, UINT_64 keySource, UINT_8 keyIndex){
...
void Ieee802154eMac::handle_MLME_SET_request(MACPIBenum pibAttribute, MAC_PIB pibAttributeValue)
{
    ...
    switch(pibAttribute)
    {
        ...
        case MAC_ACK_WAIT_DURATION:
            {
                ...
                if (((ppib.phyCurrentChannel <= 10)&&(pibAttributeValue.macAckWaitDuration != 120))
                        || ((ppib.phyCurrentChannel > 10)&&(pibAttributeValue.macAckWaitDuration != 54)))
                {
        ...
        case MAC_BATT_LIFE_EXT_PERIODS:
            {
                PLME_GET_request(PHY_CURRENT_CHANNEL);   //value will be returned in ppib
                if (((ppib.phyCurrentChannel <= 10)&&(pibAttributeValue.macBattLifeExtPeriods != 8))
                        || ((ppib.phyCurrentChannel > 10)&&(pibAttributeValue.macBattLifeExtPeriods != 6)))
                {
                    t_status = mac_INVALID_PARAMETER;
                }
                else
        ...
        case MAC_SYNC_SYMBOL_OFFSET:
            {
                PLME_GET_request(PHY_CURRENT_CHANNEL);   //value will be returned in ppib
                if (((ppib.phyCurrentChannel <= 10)&&(pibAttributeValue.macSyncSymbolOffset > 0x400))
                        || ((ppib.phyCurrentChannel > 10)&&(pibAttributeValue.macSyncSymbolOffset > 0x100)))
                {
                    t_status = mac_INVALID_PARAMETER;
                }
                else
        ...
        case MAC_FRAME_COUNTER:
            {
                if (((mpib.macFrameCounterMode == 0x04)&&(pibAttributeValue.macFrameCounter > 0xffffffff))
                        || ((mpib.macFrameCounterMode == 0x05)&&(pibAttributeValue.macFrameCounter > 0xffffffffff)))
                 {
                     t_status = mac_INVALID_PARAMETER;
                 }
                 else
...
void Ieee802154eMac::MLME_SYNC_LOSS_indication(MACenum lossReason, UINT_16 panId, UINT_8 channelNumber, UINT_8 channelPage,
                                            UINT_8 securityLevel, UINT_8 keyIdMode, UINT_64 keySource, UINT_8 keyIndex)
{
    Ieee802154eNetworkCtrlInfo *primitive = new Ieee802154eNetworkCtrlInfo();
...
void Ieee802154eMac::handle_MLME_POLL_request(Ieee802154eAddrMode coordAddrMode, UINT_16 coordPANId, IE3ADDR coordAddress,
                                            UINT_8 securityLevel, UINT_8 keyIdMode, UINT_64 keySource, UINT_8 keyIndex)
{
 ...
 *
 * param[in]*/
void Ieee802154eMac::MLME_BEACON_REQUEST_indication(Ieee802154eBeaconType beaconType, UINT_8 srcAddrMode, IE3ADDR srcAddr,
 ...
 *
 * param[in] */
void Ieee802154eMac::handle_MLME_SET_LINK_request(Ieee802154eOperation operation, UINT_16 linkHandle, UINT_8 slotframeHandle,
        UINT_16 timeslot, UINT_16 channelOffset, MACTSCHLinkOptions linkOptions, MACTSCHLinkType linkType, UINT_16 nodeAddr)
{
...
/**@author: 2014    Stefan Reis
 * brief: MLME-LLDN-DISCOVERY.request switches the LLDN into the Discover state.
 *
 * The MLME-LLDN-DISCOVERY.request primitive is generated by the next higher layer of an LLDN coordinator
 * and issued to its MLME to switch the LLDN into the Discovery state as described in 5.1.9.2.
 *
 * note: only for LLDN, see Std 802.15.4e-2012 (6.2.20.2) page 150 */
void Ieee802154eMac::handle_MLME_LLDN_DISCOVERY_request()
{
...
int Ieee802154eMac::calFrmByteLength(Ieee802154eFrame* frame) // TODO: extend it with the 802.15.4e frames [SR]
{
    ...
    if (frmType == Ieee802154e_BEACON)       // Std 802.15.4-2006 (figure 10) page 21
    {
        ...
        else
        {
            ...
            byteLength = MHRLength + 2 + (beaconFrm->getGtsField().gtsDescrCnt == 0?1:2)
    ...
    else if (frmType == Ieee802154e_ACK)    // Std 802.15.4-2006 (figure 12) page 22
    {
        ...
        else // Acknowledge frame - Std 802.15.4-2006/2011
...
bool Ieee802154eMac::getPANIDComp(FrameCtrl frmCtrl, UINT_16 srcPanId, UINT_16 dstPanId)
{
            ...
            && srcPanId == 0 && dstPanId == 0)
    {
        return true;
            ...
            && frmCtrl.frameVersion == Ieee802154_compatible
            && srcPanId == 0 && dstPanId == 0
            && frmCtrl.frameType != Ieee802154e_MULTI)
    {
        return true;
    }
    else if (frmCtrl.srcAddrMode != defFrmCtrl_AddrModeNone
...
void Ieee802154eMac::handleGtsTimer()
{
    ...
    if (isPANCoor)
    {
        ...
        EV << "allocated for MAC address = " << (int)gtsList[indexCurrGts].devShortAddr << ", startSlot = "
                << (int)gtsList[indexCurrGts].startSlot << ", length = " << (int)gtsList[indexCurrGts].length
                << ", isRecvGTS = " << gtsList[indexCurrGts].isRecvGTS << ", isTxPending = "
                << gtsList[indexCurrGts].isTxPending << endl;
...
macLinkTableEntry* Ieee802154eMac::getLinkEntry(UINT_64 asnCounter)
{
    if (mpib.macTSCHenabled)
    {
        ...
        for (int i = 0; i < slotframeTable->getNumSlotframes(); i++)
        {
           ...
           tmpLink = linkTable->getLinkByTimeslotSlotframe((asnCounter) % tmpSlotframe->getSlotframeSize(),i);
           if (tmpLink != NULL)
           {
        ...
        {   // filter the links - Std 802.15.4e-2012 (5.1.1.5.4 Multiple slotframes) page 18
            ...
            for (std::list<macLinkTableEntry*>::iterator i = tmpLinkVector.begin(); i != tmpLinkVector.end(); ++i)
            {
                ...
                {
                    if (tmpEntry->getLinkOption() > (*i)->getLinkOption()
                            || (tmpEntry->getLinkOption() == (*i)->getLinkOption() && tmpEntry->getSlotframeId() > (*i)->getSlotframeId()))
                        tmpEntry = (*i);
                }
            }
            return tmpEntry;
                    ...
                    for (it = tmpMLME->ieMLMESub.begin(); it!=tmpMLME->ieMLMESub.end(); ++it)
                    {
                        if ((*it)->ieMLMEtype == false) //short
                        {
                            ...
                            else if ((*it)->ieMLMESubID == IE_MLME_SHORT_TSCH_TIMESLOT)
                            {
                                ...
                                if (tmpTimeslot->getTemplateId() + 1 > timeslotTable->getNumTemplates() )
    ...
    if (mpib.macTSCHenabled && !taskP.taskStatus(TP_TS_TX_CCA_TSCHCCA) && !taskP.taskStatus(TP_TS_RX_CCA_TSCHCCA))
    {
        ...
        if (linkOption == LNK_OP_TRANSMIT || linkOption == LNK_OP_SHARED) // TSCH CCA algorithm OR TSCH CSMA-CA algorithm for shared links
        {
            ...
            if (txData != NULL)
            {
                ...
                string tmpTxt = (txData->getFrmCtrl().frameType==Ieee802154e_DATA)?txData->getEncapsulatedPacket()->getName():frameTypeTxt[txData->getFrmCtrl().frameType];
                EV << "[TSCH CCA]-Transmitter: "<< retries <<" retries for this " << txData->getName() << "("
                        << tmpTxt << ") frame" << endl;

                if (tschSharedLink && backoffMethod == EXPONENTIAL
                        && retries <= mpib.macMaxFrameRetries
                        && retries > 0
                        && activeNeighborEntry != NULL)
                { // in TSCH mode the device shall use an exponential backoff mechanism - Std 802.15.4e-2012 (5.1.1.4.3 TSCH CSMA-CA algorithm) page 14
                    ...
                    else // for the case td == 0, to have the td == -1 for the recalculation of the transmission delay
