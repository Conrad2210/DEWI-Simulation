...
void Node::handleMessage(cMessage *msg)
{
	...
	{                           // self message
		if (msg->getKind() == SMSG_INIT)
		{
			...
			ev << this->myId << " my chosen no. is :" << p
					<< " Threshhold is : " << t;
		...
		else if (msg->getKind() == SMSG_STATUS2BS)
		{
			...
			send2BS(this->myId, this->bsId, this->energy, this->myStatus,
					this->myCluster, this->xpos, this->ypos);
			this->sentBS++;
		}
		else if (msg->getKind() == SMSG_CHK)
		{
		...
		else if (msg->getKind() == SMSG_CONN)
		{
			...
			for (i = 1; i <= simulation.getLastModuleId(); i++)
			{
						...
						|| (strcmp(mod->getName(), "bs") == 0))
				{
					...
					else if (strcmp(mod->getName(), "bs") == 0)
					{
						id = ((BS *) mod)->myId;
						this->bsId = id;
						x = ((BS *) mod)->xpos;
						y = ((BS *) mod)->ypos;
						this->bsDist = (x / 10 - this->xpos / 10)
								* (x / 10 - this->xpos / 10)
								+ (y / 10 - this->ypos / 10)
										* (y / 10 - this->ypos / 10);
					}
					if (id != this->myId /*&& id == this->bsId*/)
					{
						...
						ev << this->myId << "(" << this->xpos << ","
								<< this->ypos << ") found node with id " << id
								<< " xpos: " << x << " ypos: " << y << "\n";
		...
		else if (msg->getKind() == SMSG_AH)
		{
			...
			for (i = 1; i <= simulation.getLastModuleId(); i++)
			{
				...
				if ((this->cHead == 1) && (strcmp(mod->getName(), "bs") == 0))
				{
					id = ((BS *) mod)->myId;
					this->bsId = id;
					x = ((BS *) mod)->xpos;
				...
				if (id == this->headId)
				{
					...
					ev << this->myId << "(" << this->xpos << "," << this->ypos
							<< ") found node with id " << id << " xpos: " << x
							<< " ypos: " << y << "\n";
...
void Node::send2BS(int src, int rec, int energy, int status, int cluster,
		int xpos, int ypos)
{
...
void Node::sendTDMA()
{
	...
	for (n = 2; n < this->nrNodes + 2; n++)
	{
		if (n != this->myId && nodePtr[n]->headId == this->myId)
		{
			...
			nodePtr[n]->headDist = (((nodePtr[n])->xpos / 10 - this->xpos / 10)
					* ((nodePtr[n])->xpos / 10 - this->xpos / 10))
					+ (((nodePtr[n])->ypos / 10 - this->ypos / 10)
							* ((nodePtr[n])->ypos / 10 - this->ypos / 10));
